...
class FileCache():
    def __init__(self, available_area, using_area):
        ...
        for area in self.available_area:
            if area is not using_area:
                self.no_using_area[area] = self.getEngine(using_area=area)
    ...
    def set(self, key, file_path, del_local=True, set_all_area=False):
        if set_all_area:
            self.default_engine.set_namespace(namespace=self.namespace).set(key=key, file_path=file_path,
                                                                            del_local=del_local)
            for area in self.no_using_area:
                self.no_using_area[area].set_namespace(namespace=self.namespace).set(key=key,
                                                                                     file_path=file_path,
                                                                                     del_local=del_local)
        else:
    ...
    def delete(self, key, delete_all_area=False):
        if delete_all_area:
            ...
            for area in self.no_using_area:
                self.no_using_area[area].set_namespace(namespace=self.namespace).delete(key=key)
            return True
        else:
