...
from azureml.contrib.automl.dnn.vision.common import constants
from azureml.contrib.automl.dnn.vision.common.trainer.lrschedule_parameters import StepLRSchedulerParameters, \
...
class BaseLRSchedulerWrapper(ABC):
    ...
    def __init__(self, optimizer, batches_per_epoch, settings):
        """
...
class LRSchedulerFactory:
    ...
    def get_lr_scheduler(self, optimizer, batches_per_epoch, settings):
        ...
        lr_scheduler_type = settings[constants.TrainingLiterals.LR_SCHEDULER]

        if lr_scheduler_type not in LRSchedulerFactory._scheduler_dict:
            raise ClientException('{} not supported'.format(lr_scheduler_type))\
                .with_generic_msg("Scheduler type not supported.")

        return LRSchedulerFactory._scheduler_dict[lr_scheduler_type](optimizer, batches_per_epoch, settings)


def setup_lr_scheduler(optimizer, batches_per_epoch, settings):
