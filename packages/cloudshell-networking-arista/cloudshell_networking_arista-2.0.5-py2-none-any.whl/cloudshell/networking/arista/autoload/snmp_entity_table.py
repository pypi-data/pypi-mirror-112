...
class SNMPEntityTable(object):
    ...
    ENTITY_VENDOR_TYPE_TO_CLASS_MAP = {
        "cevcontainer": "container",
        "cevchassis": "chassis",
        "cevmodule": "module",
        "cevport": "port",
        "cevpowersupply": "powerSupply",
    }
    ...
    def __init__(self, snmp_handler, logger, if_table):
        ...
        self.port_exclude_pattern = (
            r"stack|engine|management|mgmt|voice|foreign|cpu|control\s*ethernet\s*port"
        )
        self.entity_to_container_pattern = (
            "powershelf|cevsfp|cevxfr|cevxfp|cevContainer10GigBasePort|"
            "cevModulePseAsicPlim"
        )
    ...
    def chassis_list(self):
        if not self._filtered_chassis_list:
            if len(self._chassis_list) < 1:
                ...
                raise Exception(
                    "Cannot load entPhysicalTable. Autoload cannot continue"
                )
    ...
    def _get_sorted_modules_with_ports(self):
        ...
        module_relative_paths = sorted(
            self.relative_address, key=self.relative_address.get
        )
        self._sorted_module_list = [
            module
            for module in module_relative_paths
            if module in self._module_list and module not in self.exclusion_list
        ]
    ...
    def _analyze_module(self, module):
        if module not in self.exclusion_list:
            ...
            while module_rel_path in self.relative_address.values():
                i += 1
                module_rel_path = "{0}/{1}".format(
                    module_parent_address, (int(self.get_resource_id(module)) + i)
                )
            self.relative_address[module] = module_rel_path
            self._module_list.append(module)
            self._logger.debug(
                "Added {0} with relative path {1}".format(
                    self._entity_table[module]["entPhysicalDescr"], module_rel_path
                )
            )
        else:
    ...
    def _get_entity_table(self):
        ...
        Elements are ports, modules, chassis, etc.
        :rtype: QualiMibTable
        ...
        for index in physical_indexes.keys():
            ...
            temp_entity_table.update(
                self._snmp.get_properties(
                    "ENTITY-MIB", index, entity_table_critical_port_attr
                )[index]
            )
            ...
            if (
                ...
                or "other" in temp_entity_table["entPhysicalClass"]
            ):
                index_entity_class = self.ENTITY_VENDOR_TYPE_TO_CLASS_MAP.get(
                    vendor_type
                )
            ...
            temp_entity_table.update(
                self._snmp.get_properties(
                    "ENTITY-MIB", index, entity_table_optional_port_attr
                )[index]
            )
            ...
            if re.search(
                ...
                self._logger.debug(
                    "Successfully loaded '{0}'".format(
                        temp_entity_table["entPhysicalDescr"]
                    )
                )
            else:
                continue
            ...
            if temp_entity_table["entPhysicalClass"] == "chassis":
                ...
                if not re.search(
                    self.port_exclude_pattern,
                    temp_entity_table["entPhysicalName"],
                    re.IGNORECASE,
                ) and not re.search(
                    self.port_exclude_pattern,
                    temp_entity_table["entPhysicalDescr"],
                    ...
                    port_entity = self._get_mapping(
                        index, temp_entity_table[self.ENTITY_PHYSICAL]
                    )
    ...
    def _filter_lower_bay_containers(self):
        """Filter rare cases when dev have multiple bays with sep cont in each bay."""
        upper_container = None
        lower_container = None
        containers = (
            self._entity_table.filter_by_column("Class", "container")
            .sort_by_column("ParentRelPos")
            .keys()
        )
        ...
        if lower_container and upper_container:
            child_upper_items_len = len(
                self._entity_table.filter_by_column("ContainedIn", str(upper_container))
                .sort_by_column("ParentRelPos")
                .keys()
            )
            child_lower_items = (
                self._entity_table.filter_by_column("ContainedIn", str(lower_container))
                .sort_by_column("ParentRelPos")
                .keys()
            )
            for child in child_lower_items:
                self._entity_table[child]["entPhysicalContainedIn"] = upper_container
                self._entity_table[child]["entPhysicalParentRelPos"] = str(
                    child_upper_items_len
                    + int(self._entity_table[child]["entPhysicalParentRelPos"])
                )
    ...
    def get_resource_id(self, item_id):
        ...
        if parent_id > 0 and parent_id in self._entity_table:
            if re.search(
                r"container|backplane",
                self._entity_table[parent_id]["entPhysicalClass"],
            ):
    ...
    def get_relative_address(self, item_id):
        ...
        if int(item_id) not in self._filtered_chassis_list:
            ...
            if parent_id not in self.relative_address.keys():
                ...
                if result != "":
                    result = "{}/{}".format(
                        self.get_relative_address(parent_id), result
                    )
                else:
    ...
    def _filter_entity_table(self, raw_entity_table):
        """Filters out all elements.

        Check  if their parents, doesn't exist, or listed in self.exclusion_list
        ...
        for element in reversed(elements):
            ...
            if (
                parent_id not in raw_entity_table
    ...
    def _get_module_parents(self, module_id):
        ...
        if parent_id > 0 and parent_id in self._entity_table:
            if re.search(r"module", self._entity_table[parent_id]["entPhysicalClass"]):
                ...
                r"chassis", self._entity_table[parent_id]["entPhysicalClass"]
            ):
                return result
            else:
                result.extend(self._get_module_parents(parent_id))
    ...
    def _get_mapping(self, port_index, port_descr):
        ...
        try:
            ent_alias_mapping_identifier = self._snmp.get(
                ("ENTITY-MIB", "entAliasMappingIdentifier", port_index, 0)
            )
            port_id = int(
                ent_alias_mapping_identifier["entAliasMappingIdentifier"].split(".")[-1]
            )
            port_if_entity = self._if_table.get_if_entity_by_index(port_id)
        except Exception as e:
            self._logger.error(
                "Failed to load entAliasMappingIdentifier: {}".format(e.message)
            )

            port_if_entity = self._if_table.get_if_index_from_port_name(
                port_descr, port_exclude_list
            )
