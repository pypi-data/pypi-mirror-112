...
def list(source: str) -> bool:
    ...
    return acp_list(source=StatusDataSource(source), should_display=True).success
...
def search(query: str, source: str) -> bool:
    ...
    return acp_search(query=query, source=StatusDataSource(source), should_display=True).success


@acp.command(cls=KSDKCommand)
@click.argument("acp_name", nargs=1, type=click.STRING)
@click.option("--source", type=Choice(StatusDataSource.as_list()), required=False, help=KSDKHelpMessages.status_source)
def show(acp_name: str, source: str) -> bool:
    """
    ...
    return acp_show(acp_name=acp_name, source=StatusDataSource(source), should_display=True).success


@acp.command(cls=KSDKCommand)
@click.argument("acp_name", nargs=1, type=click.STRING)
@click.option("-y", "--yes", default=False, is_flag=True, show_default=True, help=KSDKHelpMessages.yes)
def delete(acp_name: str, yes: bool) -> bool:
    """
    Delete an ACP from the platform.

    e.g. kelvin acp delete "my-acp"

    """
    from kelvin.sdk.interface import acp_delete

    return acp_delete(acp_name=acp_name, ignore_destructive_warning=yes).success
...
def provision_script() -> bool:
    ...
    return acp_provision_script().success
