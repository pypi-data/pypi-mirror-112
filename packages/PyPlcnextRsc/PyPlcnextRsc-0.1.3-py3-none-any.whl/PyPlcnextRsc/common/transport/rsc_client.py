...
class _Authentication:
    ...
    def __init__(self, rscClient):
        ...
        self.logger = logging.getLogger(__name__ + "." + self.__class__.__name__)
        self.rscClient = rscClient
        self.SecuritySessionInfoService2 = ISecuritySessionInfoService2(rscClient)
        self.passwordAuthenticationService = IPasswordAuthenticationService(rscClient)
        ...
        if require_authentication:
            if not rscClient.info.isAuthenticationProvided():
...
class RemotingReader:
    ...
    def __init__(self, binary_reader, rsc_reader, rsc_client_context):
        self.rscReader = rsc_reader
        self.binaryReader = binary_reader
        self.rscClientContext = rsc_client_context
...
class RemotingWriter:
    def __init__(self, binary_writer, rsc_writer, rsc_client_context: RscClientContext):
        self.rscWriter = rsc_writer
        self.binaryWriter = binary_writer
        self.rscClientContext = rsc_client_context
