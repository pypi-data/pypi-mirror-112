    ...
    for symbol in NEED_SPLIT:
        ...
        for identifier in Ids:
            if symbol in identifier and len(identifier) > s_len:
                left, right = identifier.split(symbol, 1)
                if left:
                    tmp.append(left)
                tmp.append(symbol)
                if right:
...
def _receiveArray(schema: _ArraySchema, value):
    ...
    if should_element_rsc_type == RscType.Array:
        ...
        inner = None
        for current in value:  # don't care the syntax warning
            inner = _receiveStruct(schema.next_dimension_or_struct, current)
            ret.append(inner)
...
def _receiveStruct(schema: _StructSchema, value):
    ...
    for idx in range(should_field_count):
        ...
        if isinstance(current_schema, _ArraySchema):
            ...
            _values[current_schema.name] = _receiveStruct(current_schema, current_value.GetValue())
        else:
...
def ReceiveAsSchemaInstance(schema, variant: RscVariant):
    """
...
class DataTypeStore:
    ...
    def ReceiveAsSchemaInstance(self, data_type_name: str, variant: RscVariant):
        """
        ...
        return ReceiveAsSchemaInstance(schema, variant)
