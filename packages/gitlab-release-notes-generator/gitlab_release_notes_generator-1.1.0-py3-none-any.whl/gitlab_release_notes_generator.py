...
def create_markdown(date, issues, merge_requests, milestone=""):
...
def create_release_notes(server, project_id, auth, labels, milestone):
    ...
    if milestone != "":
        ...
        merged_merge_requests = gitlab_api.get_merged_merge_requests_for_project_for_milestone(labels=labels, milestone=milestone)
    else:
        closed_issues = gitlab_api.get_closed_issues_for_project(updated_before, updated_after, labels=labels)
        merged_merge_requests = gitlab_api.get_merged_merge_requests_for_project(updated_before, updated_after, labels=labels)

    notes = create_markdown(formatted_date, closed_issues, merged_merge_requests, milestone)
    return notes
...
def main():
    ...
    parser.add_argument(
        '--server', default='https://gitlab.com/', help='Url of gitlab server (default: https://gitlab.com/)'
    )
    ...
    try:
        ...
        notes = create_release_notes(server=server, project_id=project_id, auth=auth, labels=labels, milestone=milestone)
    except Exception as e:
        logger.error(str(e))
        raise
