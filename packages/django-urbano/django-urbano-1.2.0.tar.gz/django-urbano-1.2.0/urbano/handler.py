...
class UrbanoHandler:
    ...
    def get_default_payload(self, instance):
        ...
        try:
            zone_code = instance.address.commune.zone_code
            ...
            payload = {
                ...
                'cedibles': [],
                'cod_barra': '',
                'cod_cliente': instance.customer_rut,
                'cod_rastreo': instance.vtex_reference,
                'correo_elec': '',
                'description': '',
                'dir_entrega': instance.address.full_address,
                'fech_emi_vent': instance.created_at.strftime('%d/%m/%Y'),
                'fech_pro': instance.shipping_date.strftime('%d/%m/%Y'),
                'fech_venc': (instance.shipping_date + timedelta(days=14)).strftime('%d/%m/%Y'),
                ...
                'moneda': 'CLP',
                'monto_asegurado': '',
                'nom_autorizado': instance.customer_full_name,
                'nom_autorizado_2': '',
                'nom_cliente': instance.customer_full_name,
                'nom_empresa': '',
                'nom_urb': '',
                'nro_doc_autorizado': instance.customer_rut,
                'nro_doc_autorizado_2': '',
                'nro_factura': '',
                'nro_guia_trans': instance.wms_reference,
                'nro_int': instance.address.unit,
                'nro_o_compra': instance.nbr_reference,
                'nro_telf': '',
                'nro_telf_mobil': instance.customer_phone,
    ...
    def create_shipping(self, data):
        ...
        try:
            ...
            message_error = response.get('mensaje') or response.get('msg_error')
