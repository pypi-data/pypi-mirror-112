...
if not dev_published_versions:
    cur_dev_version = f"{MAJOR_VERSION}.{MINOR_VERSION}.{env}0"
else:
    cur_dev_version = dev_published_versions[0]

prod_published_versions = list(reversed(sorted(requests.get(prod_pypi_url).json()['releases'].keys(), key=StrictVersionExt)))
if not prod_published_versions:
    cur_prod_version = f"{MAJOR_VERSION}.{MINOR_VERSION}.0"
else:
    cur_prod_version = prod_published_versions[0]
