...
### set version
published_versions = list(reversed(sorted(requests.get(pypi_url).json()['releases'].keys(), key=StrictVersionExt)))
if not published_versions:
    current_version = f"{MAJOR_VERSION}.{MINOR_VERSION}.0"
    if not env == 'prod':
        current_version += f".{env}0"
else:
    current_version = published_versions[0]

current_major, current_minor, current_patch, current_prerelease, current_prerelease_num = parse_version(current_version)
...
if new_major < current_major:
    raise Exception(f"The new major version '{new_major}' cannot be smaller than the current one '{current_major}'.")
elif new_major == current_major:
    if new_minor < current_minor:
        raise Exception(f"The new minor version '{new_minor}' cannot be smaller than the current one '{current_minor}'.")
        ...
        ### no prerelease
            ...
            if new_patch <= current_patch:
                raise Exception(f"The new version '{new_major}.{new_minor}.{new_patch}' must be greater than the current one '{current_major}.{current_minor}.{current_patch}'.")
...
if new_prerelease:
    new_version += f".{new_prerelease}{new_prerelease_num}"
