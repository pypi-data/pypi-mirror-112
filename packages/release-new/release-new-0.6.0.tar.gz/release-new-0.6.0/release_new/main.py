...
def main():
    ...
    if args.preview_changelog:
        ...
        print(generate_changelog("upcoming release", format=changelog_format).strip())
        return

    try:
        do_release(args.release_type, args.no_previous_tag_check, dry_run=args.dry_run)
...
def do_release(release_type, no_previous_tag_check, dry_run):
    ...
    changelog_content = (
        generate_changelog(new_version, format=changelog_format).rstrip()
        + "\n\n"
