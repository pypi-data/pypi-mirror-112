...
class render(object):
    def __init__(self,stdata,storidata,dbdata):
        if "help" in stdata:
            print(json.dumps(dbdata,ensure_ascii=False,skipkeys=False,indent=4))
        tpl=""  #模板
        k=False
        for l in storidata:
            ...
            if k:
                tpl=tpl+l
        nt=jinja2.Template(tpl).render(dbdata)  #新的文本
        lnt=nt.split("\n")
        if not re.search(stdata["start"],lnt[0]):nt=stdata["start"]+"\n"+nt
        if not re.search(stdata["end"],lnt[0]):nt=nt+stdata["end"]
        nt=nt+"\n"
        f=open(stdata["file"])
        ft=f.readlines()
        newline=f.newlines
        ...
        for i in range(len(ft)):
            if ls<0 and re.search(stdata["start"],ft[i]):
                ls=i
            if ls>=0 and re.search(stdata["end"],ft[i]):
                le=i
                if le>ls:
                    ot=ot+ft[i]
                else:
        ...
        if ot==nt:return
        if ot=="":
            print("render error, check the setup info.")
            return
