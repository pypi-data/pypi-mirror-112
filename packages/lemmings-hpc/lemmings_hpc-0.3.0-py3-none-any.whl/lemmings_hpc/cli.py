...
def lemmings_cli(workflow, status, yaml, noverif, parallel):
    """
    ...
    # from lemmings.chain.database import Database
    # from lemmings.chain.machine import Machine
    # from lemmings.chain.chain import Lemmings
    # from lemmings.chain.chain import _handle_exception
    # from lemmings.chain.lemmingjob_base import LemmingsStop
...
def lemmings_status(database):
    ...
    try:
        _ = [print(string) for string in db.get_current_status()]
    except ValueError as excep:
