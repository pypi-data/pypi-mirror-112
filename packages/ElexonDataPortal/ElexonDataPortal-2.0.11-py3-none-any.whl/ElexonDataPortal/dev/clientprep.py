...
def construct_request_type_filter(
    ...
    request_type_filter = {
        ...
        'just_date': (has_SP == 0) and (has_date == 1),
...
def check_request_type_filter(
    ...
    * filter does not contain only one request type
    ...
    assert (has_start_date + has_end_date == 2) or (has_SP + has_date != 1) or (has_SP == 0), 'SP should not be provided on its own' + filter_str
