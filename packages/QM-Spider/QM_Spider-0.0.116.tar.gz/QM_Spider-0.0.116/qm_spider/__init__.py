...
class Get_App_Keyword:
    ...
    def get_keywordSummary(self):
        """
            ...
            * 覆盖词上方的汇总表，默认获取日期为当日；
        """
        url = 'https://api.qimai.cn/app/keywordSummary?country=%s&appid=%s&version=%s&sdate=%s&device=%s&edate=' %(self.country, self.appid, self.version, self.start_date, self.device)
        res = session.get(url, headers=headers)
