...
def _process_details_insert_or_update(expid, exp_id, current_details, conn):
    ...
    if exp_id:        
        ...
        if current_details:
            # Update
            result = DbRequests._update_ecearth_details(conn, exp_id, user, created, model, branch, hpc)            
        else:
            # Insert
            _Id = DbRequests._insert_into_ecearth_details(conn, exp_id, user, created, model, branch, hpc)
