...
class ExperimentGraphDrawing(MainDataBase):
    ...
    def calculate_drawing(self, allJobs, independent=False):
        """
        Called in a thread.
        ...
        try:
            ...
            with portalocker.Lock(os.path.join(self.folder_path, lock_name), timeout=1) as fh:
                self.conn = self.create_connection(self.database_path)
                monitor = Monitor()
                ...
                if len(allJobs) > 2200:
                    result = graph.create(
                        ['dot', '-Gnslimit=2', '-Gnslimit1=2', '-Gmaxiter=100', '-Gsplines=none', '-v'], format="plain")
                else:
