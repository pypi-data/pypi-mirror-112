...
def _construct_pre_processing_pipeline(
    ...
    timeseries_param_dict: Dict[str, Any],
    categories_by_col: Optional[Dict[str, List[Any]]] = {},
    ...
    # We split the pre calculated categories into 2 buckets one for the grain columns and one for non grain columns
    # This is required so that we feed appropriate buckets into Numericalize and GrainIndex featurizers
    # Assumption here is that categories are calculated for columns after taking into consideration FeaturizationConfig
    categories_by_grain_cols = {}
    categories_by_non_grain_cols = {}
    if categories_by_col:
        categories_by_grain_cols = {col: categories_by_col[col]
                                    for col in categories_by_col if col in grain_column_names}
        categories_by_non_grain_cols = {col: categories_by_col[col]
                                        for col in categories_by_col if col not in grain_column_names}
