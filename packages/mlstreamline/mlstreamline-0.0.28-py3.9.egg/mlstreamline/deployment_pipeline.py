...
from mlstreamline.transformers.outlier_transformer import OutlierTransformer
from mlstreamline.transformers.custom_model_transformer import CustomModelTransformer
...
class DeploymentPipeline:
    def __init__(self, config, pipeline_path=None, classifier=None, data=None, model_transformer_dict=None):
        """
        ...
        classifier : sklearn classifier object
            The object will be instantiated and used to fit the pipeline.
        ...
        self.model_transformer_dict = model_transformer_dict
        self.required_configs = ['predictor_columns', 'numeric_features', 
                                 ...
                                 'no_transform_columns', 'custom_model_features']
    ...
    def check_configs(self):
        columns_check = self.config['numeric_features'] + \
                ...
                self.config['custom_model_features'] + \
    ...
    def construct_preprocessor(self):
        ...
        custom_model_transformer = Pipeline(steps=[
                        ('imputer', SimpleImputer(strategy='most_frequent')),
                        ('custom_model_transformer', CustomModelTransformer(self.config['custom_model_features']))
                ])
        ...
        if self.model_transformer_dict:
            for model_name, model in self.model_transformer_dict:
                model_transformer = Pipeline(steps=[
                        ('imputer', SimpleImputer(strategy='median')),
                        (model_name, CustomModelTransformer(model=model))
                ])
                transformers.append((model_name + '_transformer',
                                     model_transformer,
                                     self.config[model_name + '_column']))
