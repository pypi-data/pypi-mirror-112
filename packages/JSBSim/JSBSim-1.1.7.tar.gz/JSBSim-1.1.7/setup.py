...
if 'sdist' not in dist.commands and compiler.find_library_file([library_path],
    ...
    # the sources.
    if compiler.compiler_type == 'msvc':
        compile_flags = ['/D'+flag for flag in ['_USE_MATH_DEFINES', 'NOMINMAX', 'JSBSIM_VERSION="1.1.7 [GitHub build 562/commit 6584d8702fb0907f316054419456d07bde26dfa4]"','HAVE_EXPAT_CONFIG_H',]]
        if sys.version_info < (3,9):
            compile_flags = [flag.replace('"', '\\"') for flag in compile_flags]
    else:
        compile_flags = ['-D'+flag for flag in ['JSBSIM_VERSION="1.1.7 [GitHub build 562/commit 6584d8702fb0907f316054419456d07bde26dfa4]"','HAVE_EXPAT_CONFIG_H',]]
