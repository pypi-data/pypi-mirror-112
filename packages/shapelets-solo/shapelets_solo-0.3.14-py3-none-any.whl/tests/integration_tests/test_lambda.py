...
class LambdaTest(unittest.TestCase):
    ...
    def test_map(self):
        res = dsl_op.mapTS(lambda x: x + 100, LambdaTest._sequence)
    ...
    def test_filter(self):
        ...
        res = dsl_op.filterTS(lambda x: x > 5, seq1)
        res = LambdaTest._client.run(res)
        data = LambdaTest._client.get_sequence_data(res)
    ...
    def test_filter_global(self):
        seq1 = LambdaTest._sequence
        threshold = 5
        res = dsl_op.filterTS(lambda x: x > threshold, seq1)
        res = LambdaTest._client.run(res)
        data = LambdaTest._client.get_sequence_data(res)
        self.assertIsInstance(data, pd.Series)
        self.assertEqual(data.size, 2)
    ...
    def test_reduce(self):
        ...
        seq1 = LambdaTest._sequence
        res = dsl_op.reduceTS(lambda x, y: max(x, y), seq1)
        res = LambdaTest._client.run(res)
        self.assertIsInstance(res, NDArray)
        np_array = client.get_nd_array_data(res)
        self.assertAlmostEqual(8.8, np_array[0], places=5)
