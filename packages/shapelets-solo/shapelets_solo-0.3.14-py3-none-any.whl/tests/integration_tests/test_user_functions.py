...
def my_flow(seq_a, seq_b):
    a = dsl_op.plusTSTS(seq_a, seq_b)
    b = dsl_op.timesTSTS(a, seq_b)
    c = dsl_op.plusTSTS(a, b)
    return c
...
def multi_output_array_flow(max_value: int, values: typing.List[int]):
    ...
    return max_value, dsl_op.testFilterGreaterThanValueList(max_value, values), max_value


def multi_output_array_flow_2(max_value: int, values: typing.List[int]):
    max_value = dsl_op.abs_w(max_value)
    return dsl_op.testFilterGreaterThanValueList(max_value, values), max_value


def multi_output_array_flow_3(max_value: int, values: typing.List[int]):
    max_value = dsl_op.abs_w(max_value)
    return max_value, dsl_op.testFilterGreaterThanValueList(max_value, values)
...
class UserFunctionsTest(unittest.TestCase):
    ...
    def test_input_output_list(self):
        ...
        res = client.run(dsl_op.testFilterGreaterThanValue(26, nd_array))
    ...
    def test_input_output_list_2(self):
        client = UserFunctionsTest._client
        res = client.run(dsl_op.testFilterGreaterThanValueList(26, [1, 3, 34, 4, 56, 25]))
    ...
    def test_get_function_parameters(self):
        ...
        function = client.get_function_parameters(name="absEnergy")
        self.assertEqual("absEnergy", function.name)
