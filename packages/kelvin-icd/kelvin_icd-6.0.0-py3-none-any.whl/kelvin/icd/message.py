...
class Endpoint(Model):
    ...
    acp_name: Optional[str] = Field(None, title="ACP Name", description="ACP name.")
    workload_name: Optional[str] = Field(None, title="Workload Name", description="Workload name.")

    def __str__(self) -> str:
        """String representation."""

        if not self.acp_name and not self.workload_name:
            return ""
        return f"{self.acp_name or ''}/{self.workload_name or ''}"
...
class Header(CoreHeader):
    ...
    def validate_endpoints(cls, value: Any) -> Any:
        ...
        acp_name, workload_name = value.split("/", 1)

        return {"acp_name": acp_name, "workload_name": workload_name}
