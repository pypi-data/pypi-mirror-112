...
def compute_strc_prop(adj_mad, nodes=None, dimensions=16, p=1, q=0.5,
  ...
  :param adj_mad: Adjacency matrix representation of the network, square and symmetric matrix.
  :type adj_mad: np.matrix[int]
  :param nodes: List of identifiers of nodes, must match the number of rows of the adjacency matrix.
  ...
  :type q: floar
  :param path: Relative path where the dataset will be saved, defaults to current path
  :type path: string
  ...
  :return: Dataset with scaled features representing the structural properties of the network
  :rtype: Dataframe
  :return: Labels (names) of the features representing the structural properties of the network
  :rtype: List[string]
  """
