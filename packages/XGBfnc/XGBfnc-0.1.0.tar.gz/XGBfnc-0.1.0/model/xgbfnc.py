...
class XGBfnc:
  ...
  def create_classifier(self, n_iter=5, n_jobs_cv=None, n_jobs_xgb=2,
    ...
    :param n_iter: Number of iterations in cross validation for hyper-parameters tuning, defaults to 5
    :type n_iter: int
    :param n_jobs_cv: Number of parallel jobs running for hyper-parameters tuning, defaults to None
    :type n_jobs_cv: int
    :param n_jobs_xgb: Number of parallel jobs running for training the classifier, defaults to 2
    :type n_jobs_xgb: int
    :param eval_metric: Evaluation metric for training the classifier , defaults to "aucpr"
    :type eval_metric: string
    :param scoring: Scoring metric for hyper-parameters tuning, defaults to "recall"
    :type scoring: string
    ...
    if n_jobs_cv == None: n_jobs = multiprocessing.cpu_count() // 2
  ...
  def opt_threshold(self, y_orig, y_pred):
    ...
    :return: Classification for the input array of probabilities which maximies F1 score.
    :rtype: np.array[int]
    """
  ...
  def plot_performance(self, a, b, labels=['without', 'with']):
    ...
    :param a: Predicted probabilities for the model **without** the structural properties of the network.
    :type a: np.array[float]
    :param b: Predicted probabilities for the model **including** the structural properties of the network.
  ...
  def train(self, X, n_splits=5, seed=None, n_iter=5, n_jobs_cv=None,
    ...
    :return: Predicted probabilities with the XGBoost classifier.
    :rtype: np.array[float]
    :return: Feature importance measured by total gain.
    :rtype: dict[string->float]
    :return: Best parameter combination for the classfifier.
    :rtype: dict[string->float]
    """
  ...
  def structural_test(self, n_splits=5, seed=None, log=False, csv=True):
    ...
    :type log: bool
    :param csv: Flag for saving the results of the test in a csv file, defaults to True
    :type csv: bool

    :return: Result of the structural test, True if the the structural properties improve the prediction performance, False otherwise.
    :rtype: bool
    :return: Best parameter combination for the classfifier including the structural properties.
    :rtype: dict[string->float]
    """
