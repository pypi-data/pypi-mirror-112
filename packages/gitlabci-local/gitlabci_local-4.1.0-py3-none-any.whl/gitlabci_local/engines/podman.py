...
class Podman:
    ...
    def cmd_exec(self):
        ...
        if Platform.IS_USER_SUDO:
            return 'sudo %s exec -it' % (self.__binary)
        return '%s exec -it' % (self.__binary)
    ...
    # Run, pylint: disable-msg=unused-argument
    def run(self, image, command, entrypoint, variables, network, option_sockets,
        ...
        if volumes and isinstance(volumes.get(), dict):
            for volume in volumes.get().items():
                args_volumes += ['--volume', '%s' % (Volumes.stringify(volume))]
        ...
        # Start container
        self.__exec(['start', self.__container])
