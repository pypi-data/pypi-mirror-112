...
def main():
    ...
    group.add_argument(
        '-n', dest='network', help='Configure the network mode used\n'
        'Choices: %s. Default: %s' % (', '.join(networks_enum), networks_enum[0]))
    group.add_argument('-e', dest='env', action='append',
                       help='Define VARIABLE=value, pass VARIABLE or ENV file')
    group.add_argument(
        '-E', dest='engine', help='Force a specific engine (or define %s)\n'
        'Default list: %s' % (Bundle.ENV_ENGINE, ','.join(engine_supported())))
    ...
    group.add_argument(
        ...
        'Default list: %s' % (','.join(tags_default)))
