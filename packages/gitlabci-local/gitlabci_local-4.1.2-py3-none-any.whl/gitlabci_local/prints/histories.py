...
class JobHistory(TimedHistory):
    ...
    def footer(self):
        ...
        # Footer output
        print('  %s%s %s: %s%s in %s%s%s%s' % (
            Colors.YELLOW,
            self.__SYMBOL_FOOTER,
            self.__name,
    ...
    def print(self):
        ...
        # Print result
        if self.result:
            icon = '%s%s' % (Colors.GREEN, self.__SYMBOL_SUCCESS)
            summary = '%sSuccess in %s' % (Colors.GREEN, self.duration)
        elif self.failure_allowed:
            icon = '%s%s' % (Colors.YELLOW, self.__SYMBOL_WARNING)
            summary = '%sFailure in %s' % (Colors.YELLOW, self.duration)
        elif self.result is None:
            icon = '%s%s' % (Colors.GREY, self.__SYMBOL_SKIPPED)
            summary = '%sSkipped' % (Colors.GREY)
        else:
            icon = '%s%s' % (Colors.RED, self.__SYMBOL_FAILED)
...
class PipelineHistory(TimedHistory):
    ...
    def print(self):
        ...
        # Footer
        print('  %s%s Pipeline: %s%s in %s total%s' % (
            Colors.YELLOW,
            self.__SYMBOL_PIPELINE,
