...
class BaseApplication(ApplicationInterface, metaclass=BaseApplicationMeta):
    ...
    def make_message(
        ...
        **kwargs:
        ...
        source_acp_name = _source.get("acp_name") or info.get("acp_name") or ""
        source_workload_name = _source.get("workload_name") or info.get("workload_name") or ""

        if _target is None:
            _target = {}

        acp_name = _target.get("acp_name") or ""
        workload_name = _target.get("workload_name") or ""

        query = (acp_name, workload_name, _asset_name or asset_name or "")
        defaults = (source_acp_name, source_workload_name, asset_name)
        ...
        if selectors:
            ...
            if n == 1:
                # single match
                selector, *_ = matches
                acp_name, workload_name, _asset_name = [
            ...
            acp_name, workload_name, _asset_name = [x or y for x, y in zip(query, defaults)]

        _source = {"acp_name": source_acp_name, "workload_name": source_workload_name}
        _target = {"acp_name": acp_name, "workload_name": workload_name}
