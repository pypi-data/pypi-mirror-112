...
def assert_file_in_level(files):
    for file in files:
        if file not in config.git_record.writable:
            raise click.UsageError("{} not in level {}, try to use another level".format(
...
def assert_indices(files, index):
    for file in files:
        ...
        if length <= index:
            raise click.UsageError("The record {} does not have enough record ({})"
...
@option("--name",
        help=("Name to use, instead of the computed one."
...
@flag("--upload/--no-upload",
      help=("Upload the content as soon as added."
            " People tend to forget to upload and think their data is safe on the remote."
            " The default value of True is then conservative."),
      default=True)
@pass_context
...
def prune_git_files(ctx):
    ...
    for ref in [r[1] for r in get_refs()]:
        if ref not in [record["name"] for records in config.git_record.readonly.values() for record in records]:
...
"""
    ...
    config.git_record.write()
    if pruned:
        LOGGER.info("Pruned {} records," " you might want to run the prune-git-files also.".format(pruned))
...
@git_record.command()
@option("-f", "--from", "remote", default=current_remote, type=GitRemoteType(), help="The remote to download from")
@argument('file', nargs=-1, type=CommandSettingsKeyType("git_record"))
@pass_context
...
@option("-i",
        "--index",
        help="Index of the record to take into account."
...
@option("-i", "--index", help="Index of the record to take into account." " 1 is the most recent one.", type=int)
def show(format, all, fields, index):
    ...
    index = index or 1
    locally_avail_names = {r[1] for r in get_refs()}
    ...
    def format_records(records):
        ...
        if all and len(records) > 1:
            res += ". Other records: {}".format(", ".join("{}: {}".format(index_ + 1, format_record(record))
...
@git_record.command()
@option("-t", "--to", "remote", default=current_remote, type=GitRemoteType(), help="The remote to upload to")
...
"""
    ...
    remote_names = {r[1] for r in get_refs(remote)}
    local_names = {config.git_record.readonly[fil][-1]["name"] for fil in file}
    locally_avail_names = {r[1] for r in get_refs()}
    names = local_names.copy()
    for name in local_names - locally_avail_names:
        local_file = {fil for fil in file if config.git_record.readonly[fil][-1]["name"] == name}
        names.remove(name)
        for fil in local_file:
            file.remove(fil)
            LOGGER.error("{} is not there"
    ...
    if file:
        ...
        LOGGER.status("Uploaded {}".format(", ".join(file)))
...
def download(ctx, file, remote, force):
    ...
    for name in local_names - remote_names:
        ...
        for fil in local_file:
            file.remove(fil)
            LOGGER.error("{} is not available in"
    ...
    if file:
        ...
        LOGGER.status("Downloaded {}".format(", ".join(file)))
...
def hash_restore(ctx, file):
    ...
    for fil in file:
        ...
        hash_file = os.path.join(os.path.dirname(full), basename + ".hash")
...
@argument("file", nargs=-1)
@flag("--force",
      help=("Force the insertion of the file in git,"
