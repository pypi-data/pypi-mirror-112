...
class PluginsType(ParameterType):
    ...
    def complete(self, ctx, incomplete):
        ...
        return [(plugin, load_short_help(plugin)) for plugin in choice if startswith(plugin, incomplete)]

    def convert(self, value, param, ctx):
        choice = self.getchoice(ctx)
        if value not in choice:
            self.fail('invalid choice: %s. (choose from %s)' % (value, ', '.join(choice)), param, ctx)
        return value
...
def show(fields, format, remote, location):
    ...
    if remote:
        ...
        plugins = [plugin_name(ensure_unicode(plugin)) for plugin in config.plugins.plugins.list_plugins()]
...
def install_plugin(profile, force, plugin, login, password, develop, no_deps):
    ...
    for _plugin in plugin:
        if not force and os.path.exists(plugin_location(_plugin)):
            raise click.UsageError("{} already installed in profile {}."
                                   " If you really want to install it, use --force or"
                                   " run plugin uninstall --{} {}".format(_plugin, profile.name, profile.name, _plugin))
    for _plugin in plugin:
        ...
        def install_requirements():
            ...
            requires = [line[len(require_prefix):] for line in content.splitlines() if line.startswith(require_prefix)]
...
def install(force, plugin, login, password, develop, no_deps):
    ...
    for plugin_ in plugin:
        if "/" not in plugin_:
            raise click.UsageError("{} is not a valid plugin name to install."
...
def uninstall(plugin):
    ...
    for _plugin in plugin:
        ...
        if not os.path.exists(plugin_location(_plugin)):
            ...
            continue
        LOGGER.status("Uninstalling plugin {} from profile {}".format(_plugin, profile.name))
