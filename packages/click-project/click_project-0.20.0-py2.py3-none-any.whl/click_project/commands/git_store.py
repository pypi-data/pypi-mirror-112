...
def get_refs(remote=None):
    ...
    return [
        split(line) for line in check_output(
            ...
            internal=True,
            nostderr=True,
        ).strip().replace("\t", " ").splitlines() if line.split(" ")[1].startswith("refs/git-store/")
    ]


def get_refs_names(remote=None):
    return [ref[1] for ref in get_refs(remote)]


def get_refs_dict(remote=None):
    return {ref[1]: ref[0] for ref in get_refs(remote)}
...
def git_store():
    ...
    """
    if toplevel() is None:
        raise click.UsageError("Using git store commands only makes sense in a git repository")
...
@flag("--create-hash-file/--dont-create-hash-file", help="Create a hash file, to be consumed by generated hash scripts")
...
def _save(name, file, force, create_hash_file=False):
    ...
    if not force and name in refs_dict:
        ...
        if blob == other_blob:
            ...
            raise click.UsageError("The git name {} already exists with hash {}."
...
@option("-r",
        "--remote",
        type=GitRemoteType(),
        help=("Instead of showing the file available locally,"
              " list those available to be fetched from the remote"))
@table_fields(choices=['ref', 'details'])
@table_format(default='key_value')
@flag("--with-notes", help=("Show the first line of notes on the file." " See git-store notes --help for more info"))
...
def show(remote, format, fields, with_notes, name):
    ...
    """
    refs = [r for r in get_refs(remote) if not name or "refs/git-store/" + r[1] in name]
...
@git_store.command()
@option("-f", "--from", "remote", type=GitRemoteType(), help="Remove from this remote")
@argument("name", type=LocalReferenceType())
def remove(remote, name):
    ...
    """
    call([git()] + (["push", "-d", remote] if remote else ["update-ref", "-d"]) + [name])
...
@option("-t",
        ...
        callback=remote_callback)
@option("--name", default="*", type=LocalReferenceType(notinremote=True), multiple=True)
@flag("--force/--no-force", help="Force the push (you probably won't want to do that)")
...
def restore(file, name):
    ...
    LOGGER.info("Restored git name {} in location {}".format(name[len("refs/git-store/"):], prefixfile))
