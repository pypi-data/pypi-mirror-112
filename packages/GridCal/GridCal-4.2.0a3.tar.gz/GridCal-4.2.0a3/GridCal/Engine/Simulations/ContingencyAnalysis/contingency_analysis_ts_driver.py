...
def compute_flows_numba(e, nt, nc, LODF, Flows, rates, overload_count, max_overload, worst_flows, paralelize_from=500):
    ...
    :param LODF: LODF matrix (element, failed element)
    :param Flows: base Sf matrix (time, element)
    :param rates: rates (time, element)
    :param overload_count: (output) number of overloads per element
    :param max_overload: (output)
    :param worst_flows: (output)
    :param paralelize_from: minimum number of elements to allow parallelization
    :return:
    """
