...
def login():
    ...
    if flask.session.get(config.link + '_logged_in'):
        return redirect(config.custom_url_for(MAIN_PAGE))
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            on_login(user=user)
            return redirect(config.custom_url_for(MAIN_PAGE))
    return render_template('fmd_login.html', blueprint_name=config.blueprint_name, custom_url_for=config.custom_url_for)
