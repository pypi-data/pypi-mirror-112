...
class Config(object):
    ...
    def __init__(self):
        ...
        # dependencies
        self.get_ip = None
        self.custom_url_for = None

    def inject_dependencies(self, get_ip=None, custom_url_for=None):
        """
            ...
            :param custom_url_for: a function that acts as a custom drop-in replacement of Flask's
                url_for function. This allows for better control over the URLs generated.
        """
        if get_ip:
            self.get_ip = get_ip

        if custom_url_for:
            self.custom_url_for = custom_url_for
