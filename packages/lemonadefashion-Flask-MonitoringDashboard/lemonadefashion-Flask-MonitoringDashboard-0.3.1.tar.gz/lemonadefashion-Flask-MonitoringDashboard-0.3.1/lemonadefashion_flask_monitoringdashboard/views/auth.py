...
from lemonadefashion_flask_monitoringdashboard import blueprint, config
from lemonadefashion_flask_monitoringdashboard.core.auth import on_logout, on_login, secure, is_admin, admin_secure
from lemonadefashion_flask_monitoringdashboard.database import session_scope, User
from lemonadefashion_flask_monitoringdashboard.database.auth import get_user, get_all_users
...
def login():
    ...
    if flask.session.get(config.link + '_logged_in'):
        return redirect(url_for(MAIN_PAGE, _external=True, _scheme=config.scheme))
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            on_login(user=user)
            return redirect(url_for(MAIN_PAGE, _external=True, _scheme=config.scheme))
    return render_template('fmd_login.html', blueprint_name=config.blueprint_name, scheme=config.scheme)
