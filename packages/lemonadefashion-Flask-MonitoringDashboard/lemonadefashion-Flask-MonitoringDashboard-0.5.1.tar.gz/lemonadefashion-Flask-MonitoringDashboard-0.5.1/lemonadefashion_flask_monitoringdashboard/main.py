...
def custom_url_for(*args, **kwargs):
    SCHEME = 'http'
    HOST = 'localhost:5000'
    parse_result = urlparse(url_for(*args, **kwargs, _external=True, _scheme=SCHEME))
    return '{}://{}{}?{}#{}'.format(SCHEME, HOST, parse_result.path, parse_result.query, parse_result.fragment)
...
dashboard.config.inject_dependencies(custom_url_for=custom_url_for)
...
@app.route('/')
def to_dashboard():
    return redirect(custom_url_for(dashboard.config.blueprint_name + '.login'))
