...
def config(colors=None, group_by=None):
    ...
    lemonadefashion_flask_monitoringdashboard.config.group_by = group_by
    lemonadefashion_flask_monitoringdashboard.config.timezone = pytz.timezone('UTC')

    return lemonadefashion_flask_monitoringdashboard.config
...
def dashboard(config, endpoint, view_func, rule='/'):
    ...
    lemonadefashion_flask_monitoringdashboard.bind(app, schedule=False)
