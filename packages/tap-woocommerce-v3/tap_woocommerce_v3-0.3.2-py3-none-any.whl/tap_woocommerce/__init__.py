...
def filter_order(order):
    ...
    try:
     filtered = {
        ...
        "date_paid": parser.parse(order["date_paid"]).replace(tzinfo=tzinfo).isoformat(),
        "date_completed": parser.parse(order["date_completed"]).replace(tzinfo=tzinfo).isoformat(),
