...
class READMESetup(Setup):
    def __init__(self, pipe, logger, url):
        ...
        crawler.kill() if twisted.internet.reactor.running else None
...
class ModuleCheckerSetup(Setup):
    ...
    def start_another(self, *args):
        if not ("settings.json" in self.cache and "setup.py" in self.cache):
            return self.stop()

        READMESetup(self.cache, self.logger, self.cache["README.md"])
