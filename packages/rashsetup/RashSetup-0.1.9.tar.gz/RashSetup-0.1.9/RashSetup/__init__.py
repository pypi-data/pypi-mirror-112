...
class Setup(scrapy.crawler.CrawlerProcess):
    def __init__(
            self, pipe, logger: logging.Logger, *modules
    ):
        super().__init__()
        self.pipe = pipe
        self.logger = logger
    ...
    @classmethod
    def search_module(cls, pipe, logger, module):
        handler = Setup(pipe, logger, module)

        handler.crawl(
            ModuleCheckerSpider, pipe=pipe, url=module
        )

        handler.crawl(
            ReadmeSpider, url=module, pipe=pipe
        )

        handler.start(True)

    @classmethod
    def download_module(cls, pipe, logger, module):
        handler = Setup(pipe, logger, module)

        handler.crawl(
            RepoSpider, url=module, pipe=pipe
        )

    @classmethod
    def grab_readme(cls, pipe, logger, url):
        handler = Setup(pipe, logger, url)

        handler.crawl(
            ReadmeSpider, url=url, pipe=pipe
        )
