...
class Setup:
    ...
    def close(self, *args):
        twisted.internet.reactor.callWhenRunning(twisted.internet.reactor.stop)
...
class READMESetup(Setup):
    ...
    def close(self, _, *args):
        super().close()

        if args[0]:
            self.pipe_cache["readme"] = self.cache["result"]

        self.save(
            self.pipe_cache if args[0] else self.cache["result"]
        )
...
class ModuleCheckerSetup(Setup):
    ...
    def start_another(self, *args):
        ...
        READMESetup(
            self.pipe, self.logger, self.cache["README.md"], self.cache
        )
