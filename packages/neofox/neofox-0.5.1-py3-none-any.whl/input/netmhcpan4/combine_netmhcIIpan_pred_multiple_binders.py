...
class BestAndMultipleBinderMhcII:
    ...
    def main(self, sequence, sequence_reference, alleles, set_available_mhc):
        ...
        position_xmer_sequence = np.mut_position_xmer_seq(sequence_wt=sequence_reference, sequence_mut=sequence)
        try:
            preds = np.filter_binding_predictions(position_xmer_sequence, tmp_prediction)
            # multiple binding
            list_tups = mb.transform_mhc_prediction_output(preds, mhc=MHC_II)
