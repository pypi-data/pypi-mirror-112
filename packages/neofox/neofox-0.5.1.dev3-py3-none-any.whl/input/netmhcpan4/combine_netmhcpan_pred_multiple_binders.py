...
class BestAndMultipleBinder:
    ...
    def main(self, xmer_wt, xmer_mut, alleles, set_available_mhc):
        ...
        position_xmer = np.mut_position_xmer_seq(sequence_mut=xmer_mut, sequence_wt=xmer_wt)
        preds = np.filter_binding_predictions(position_of_mutation=position_xmer, tmppred=tmp_prediction)

        # multiple binding
        list_tups = mb.transform_mhc_prediction_output(preds)
        ...
        self.directed_to_TCR = np.mutation_in_loop(position_mutation_list=position_xmer, epitope_tuple=best_epi)
        ...
        self.best4_affinity_directed_to_TCR = np.mutation_in_loop(
            position_mutation_list=position_xmer, epitope_tuple=best_epi_affinity)
        ...
        best_9mer_affinity = np.filter_for_WT_epitope_position(preds_9mer, sequence_mut=self.mhcI_affinity_epitope_9mer,
