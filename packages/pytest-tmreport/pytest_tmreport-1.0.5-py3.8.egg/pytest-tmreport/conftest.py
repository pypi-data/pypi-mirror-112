...
def pytest_addoption(parser, pluginmanager):
    report = parser.getgroup("report")
    report.addoption("--pytest-tmreport-name",  # 报告存储路径
                     default=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + '.html',  # 参数的默认值
...
def pytest_runtest_makereport(item, call):
    ...
    if report.when == "call":
        cases.append({'title': item.nodeid})
...
def generate_report(name, path='.', **kwargs):
    with open(base_dir + '/reports/' + 'template.html', 'r') as f:
...
def setup_fixture(request):
    ...
    report_name = request.config.getoption("--pytest-tmreport-name", default=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + '.html')
