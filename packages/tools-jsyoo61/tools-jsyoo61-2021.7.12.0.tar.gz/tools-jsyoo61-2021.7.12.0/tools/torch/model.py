...
class DNN_Resnet(nn.Module):
    ...
    # def __init__(self, n_input, n_hidden_list, activation_list, skip=2):
        ...
        if n_input is None:
            ...
            skip_layers = [nn.LazyLinear(n_skip_hidden_list[0])] + [nn.Linear(n_skip_hidden_list[i], n_skip_hidden_list[i+1], bias=False) for i in range(len(n_skip_hidden_list)-1)]
        else:
