...
pkg_lib_path = os.path.join(os.path.join(os.path.expanduser("~"), "Documents"), "pkg_lib.pkl")
...
def deploy(local, package_name, folder_path, port, quick_deploy):
    ...
    add_to_lib(package_name, folder_path)
        ...
        if port_is_good:
            ...
            if not dfPackages.empty:
                ...
                if package_name in dfPackages.Name.values:
                    ...
                    if not quick_deploy:
                        check_quick_deploy = input("Would you like to redeploy the package? Enter Y to confirm: ").lower()
    ...
    if local:
        ...
        if dfPackages.empty:
            if dfContainers.empty:
                ...
                click.echo("--------------------")
                # list cleanup
                if not df_local_pkgs.empty:
                    dfContainers = pd.merge(dfContainers, df_local_pkgs, left_on=['Names'], right_on=['package_name'], how='inner')
            ...
            if dfContainers.empty:
                ...
                if not df_local_pkgs.empty:
                    dfResult = pd.merge(dfResult, df_local_pkgs, left_on=['Names'], right_on=['package_name'], how='inner')
                ...
                dfResult.drop(['LastCreated', 'Name','package_name'], axis=1, inplace=True)
...
def add_to_lib(package_name, folder_path):
    if os.path.exists(pkg_lib_path):
        #Loading packages from pkg_lib_path
        df_pkg_lib = pd.read_pickle(pkg_lib_path)
        if package_name in df_pkg_lib.package_name.values:
            #package already exists.
            for index, row in df_pkg_lib.iterrows():
                if row['package_name'] == package_name and row['folder_path'] == folder_path:
                    #folder_path is the same.
                    pass
                elif row['package_name'] == package_name and row['folder_path'] != folder_path:
                    #updating folder_path.
                    df_pkg_lib.loc[index, 'folder_path'] = folder_path
        else:
            df_new_pkg_lib = pd.DataFrame({'package_name': package_name,
                            'folder_path': folder_path}, index=[0])
            df_pkg_lib = df_pkg_lib.append(df_new_pkg_lib,sort=False)
            df_pkg_lib = df_pkg_lib.reset_index(drop=True)
            df_pkg_lib.to_pickle(pkg_lib_path)
    else:
        df_pkg_lib = pd.DataFrame({'package_name': package_name,
                        'folder_path': folder_path}, index=[0])
        df_pkg_lib.to_pickle(pkg_lib_path)
        #click.echo("Your package details have been stored at: " + pkg_lib_path)
    return
...
def get_pkg_lib():
    if os.path.exists(pkg_lib_path):
        ...
        #No Package libraries exist locally.
        df_pkg_lib = pd.DataFrame()
    return df_pkg_lib

def pkg_lib_undeploy(package_name):
    df_pkg_lib = get_pkg_lib()
    df_pkg_lib = df_pkg_lib.set_index("package_name")
    df_pkg_lib = df_pkg_lib.drop(package_name)
    df_pkg_lib = df_pkg_lib.reset_index()
    df_pkg_lib.to_pickle(pkg_lib_path)

def pkg_lib_begin():
    df_pkg_lib = get_pkg_lib()
    if not df_pkg_lib.empty:
        click.echo(df_pkg_lib)
        checkRestore = input("Would you like to restore all the above previously deployed packages? Enter Y to confirm: ").lower()
        if checkRestore == 'y':
            click.echo("Restoring previously deployed packages")
            for index, row in df_pkg_lib.iterrows():
                delete_container(get_rio_url(), row['package_name'])
                deploy(local=True, package_name=row['package_name'], folder_path = row['folder_path'], port=None, quick_deploy = True)
        else:
            for index, row in df_pkg_lib.iterrows():
                delete_container(get_rio_url(), row['package_name'])
                undeploy(local=True, package_name=None, all=True)
            
    #stop(local=True, package_name=None, all=True, quick_deploy = True)