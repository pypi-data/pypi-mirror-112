...
def fake_download_impact(id, *args, **kwargs):
    with open(f"{fixtures_folder}/{id}.jsonld", encoding='utf-8') as f:
        return json.load(f)
...
@patch(F"{class_path}.download_hestia", side_effect=fake_download_impact)
@patch(f"{class_path}.get_emission_inputs_production_terms", return_value=TERMS)
