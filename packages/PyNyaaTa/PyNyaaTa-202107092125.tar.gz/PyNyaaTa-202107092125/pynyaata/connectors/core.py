...
class Cache:
    def cache_data(self, f):
        ...
        def wrapper(*args, **kwds):
            ...
            if REDIS_ENABLED:
                data = cache.get('%s.data' % key)

                if data:
                    return {
                        'data': data,
                        'on_error': False,
                        'is_more': cache.get('%s.is_more' % key)
                    }

            ret = f(*args, **kwds)

            if not connector.on_error and REDIS_ENABLED:
                cache.set('%s.data' % key, connector.data, CACHE_TIMEOUT)
                cache.set('%s.is_more' % key, connector.is_more, CACHE_TIMEOUT)
