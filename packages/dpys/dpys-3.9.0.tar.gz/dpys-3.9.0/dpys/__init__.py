...
class curse():
    async def add_banned_word(ctx, word, dir=None):
        if dir == None:
            raise DirectoryError("A directory was not provided. \nadd_banned_word(ctx, word, dir)")
        arg = word
        os.chdir(dir)
        word = word.lower()
        ...
        async with db.execute("SELECT curse FROM curses WHERE guild = ?", (guildid,)) as cursor:
            ...
            await db.commit()
        await db.close()
    async def remove_banned_word(ctx, word, dir=None):
            if dir == None:
                raise DirectoryError("A directory was not provided. \nremove_banned_word(ctx, word, dir)")
            db = await aiosqlite.connect("curse.db")
            guildid = str(ctx.guild.id)
            os.chdir(dir)
            try:
                ...
                await db.close()
    async def message_filter(message, dir=None, admin: int=1):
        if dir == None:
            raise DirectoryError("A directory was not provided. \nmessage_filter(message, dir, admin)")
            ...
            if admin != 1:
                ...
                if adminrole in message.author.roles or message.author.top_role.position > adminrole.position or message.author.bot:
                    ...
                    try:
                        ...
                        await db.close()
                        return
    async def message_edit_filter(after, dir=None, admin: int=1):
        if dir == None:
            raise DirectoryError("A directory was not provided. \nmessage_edit_filter(message, dir, admin)")
        message = after
        if message.author.bot or message.guild is None:
            return
        os.chdir(dir)
    ...
    async def clear_words(ctx, dir=None):
        if dir == None:
            raise DirectoryError("A directory was not provided. \nclear_words(ctx, dir)")
        guildid = str(ctx.guild.id)
        os.chdir(dir)
        try:
            ...
            await db.close() 
class mute_on_join():
    async def mute_add(ctx, member, dir=None):
        if dir == None:
            raise DirectoryError("A directory was not provided. \nmute_add(ctx, member, dir)")
        os.chdir(dir)
        guildid = str(ctx.guild.id)
    ...
    async def mute_remove(ctx, member, dir=None):
        if dir == None:
            raise DirectoryError("A directory was not provided. \nmute_remove(ctx, member, dir)")
        member = str(member.id)
        guildid = str(ctx.guild.id)
        os.chdir(dir)
        try:
    ...
    async def mute_on_join(member, role, dir=None):
        if dir == None:
            raise DirectoryError("A directory was not provided. \nmute_on_join(member, role, dir)")
        user = member
    ...
    async def manual_unmute_check(after, roleid, dir=None):
        if dir == None:
            raise DirectoryError("A directory was not provided. \nmanual_unmute_check(after, roleid, dir)")
        os.chdir(dir)
        if after.bot:
            return
