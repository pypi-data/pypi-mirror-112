...
class admin():
    async def mute(ctx, member, *args, role_add=1, role_remove=1, reason=None, **kwargs):
        if ctx.guild.get_role(role_add) in member.roles:
            embed = discord.Embed(color = discord.Colour.red(), description = f"{member.name}#{member.discriminator} is already muted.")
            ...
            if reason == None:
                embed = discord.Embed(color = discord.Colour.green(), description = f"Muted {member.name}#{member.discriminator}.")
                await ctx.send(embed=embed, delete_after=7)
            else:
                embed = discord.Embed(color = discord.Colour.green(), description = f"Muted {member.name}#{member.discriminator}. Reason: {reason}")
                await ctx.send(embed=embed, delete_after=7)

    async def unmute(ctx, member, *args, role_add=1, role_remove=1, **kwargs):
        if ctx.guild.get_role(role_remove) not in member.roles:
            embed = discord.Embed(color = discord.Colour.red(), description = f"{member.name}#{member.discriminator} is not muted.")
            await ctx.send(embed=embed, delete_after=5)
        else:
            await member.remove_roles(ctx.guild.get_role(role_remove))
            if role_add != 1:
                await member.add_roles(ctx.guild.get_role(role_add))
            embed = discord.Embed(color = discord.Colour.green(), description = f"Unmuted {member.name}#{member.discriminator}.")
            await ctx.send(embed=embed, delete_after=7)
    ...
    async def clear(ctx, amount=None):
        ...
        purged = len(purged)
        embed = discord.Embed(color = discord.Colour.green(), description = f"Cleared {purged} messages.")
        await ctx.send(embed=embed, delete_after=5)
        return purged
    ...
    async def kick(ctx, member, reason=None):
        ...
        if reason is None:
            embed = discord.Embed(color = discord.Colour.green(), description = f"Kicked {member.name}#{member.discriminator}.")
            await ctx.send(embed=embed, delete_after=7)
        else:
            embed = discord.Embed(color = discord.Colour.green(), description = f"Kicked {member.name}#{member.discriminator}. Reason: {reason}")
            await ctx.send(embed=embed, delete_after=7)

    async def ban(ctx, member, reason=None):
        await member.ban(reason=reason)
        if reason is None:
            embed = discord.Embed(color = discord.Colour.green(), description = f"Banned {member.name}#{member.discriminator}.")
            await ctx.send(embed=embed, delete_after=7)
        else:
            embed = discord.Embed(color = discord.Colour.green(), description = f"Banned {member.name}#{member.discriminator}. Reason: {reason}")
            await ctx.send(embed=embed, delete_after=7)

    async def softban(ctx, member, reason=None):
        await member.ban(reason=reason)
        if reason is None:
            embed = discord.Embed(color = discord.Colour.green(), description = f"Soft banned {member.name}#{member.discriminator}.")
            await ctx.send(embed=embed, delete_after=7)
        else:
            embed = discord.Embed(color = discord.Colour.green(), description = f"Soft banned {member.name}#{member.discriminator}. Reason: {reason}")
            await ctx.send(embed=embed, delete_after=7)
        await member.unban()
    ...
    async def unban(ctx, member):
        ...
        for ban_entry in banned_users: 
            ...
            if (user.name, user.discriminator) == (member_name, member_discriminator):
                ...
                embed = discord.Embed(color = discord.Colour.red(), description = f"Unbanned {member.name}#{member.discriminator}.")
                await ctx.send(embed=embed, delete_after=7)
            else:
                embed = discord.Embed(color = discord.Colour.red(), description = f"{member.name}#{member.discriminator} is not banned.")
                await ctx.send(embed=embed, delete_after=5)
class curse():
    async def add_banned_word(ctx, word, dir):
        ...
        async with db.execute("SELECT curse FROM curses WHERE guild = ?", (guildid,)) as cursor:
            ...
            for x in word:
                if word.count(x) > 1:
                    embed = discord.Embed(color = discord.Colour.red(), description = "Words cannot be added twice.")
                    await ctx.send(embed=embed, delete_after=5)
                    return
                await db.execute("INSERT INTO curses (curse,guild) VALUES (?,?)", (x,guildid))
            await db.commit()
        await db.close()
        embed = discord.Embed(color = discord.Colour.green(), description = "Added to the list.")
        await ctx.send(embed=embed, delete_after=7)
    async def remove_banned_word(ctx, word, dir):
            ...
            try:
                ...
                if in_db == False:
                    if len(word) > 1:
                        embed = discord.Embed(color = discord.Colour.red(), description = "None of those words are in the list")
                        await ctx.send(embed=embed, delete_after=5)
                    else:
                        embed = discord.Embed(color = discord.Colour.red(), description = "That word is not in the list.")
                        await ctx.send(embed=embed, delete_after=5)
                    return 
                for x in word:
                    ...
                    await db.commit()
                await db.close()
                embed = discord.Embed(color = discord.Colour.green(), description = "Removed from the list.")
                await ctx.send(embed=embed, delete_after=5)
            except:
                embed = discord.Embed(color = discord.Colour.red(), description = "A list has not been created yet.")
                await ctx.send(embed=embed, delete_after=5)
                await db.close()
...
class warnings():
    async def warn(ctx, member, dir, reason=None):
        ...
        await db.close()
        if reason == None:
            embed = discord.Embed(color = discord.Colour.green(), description = f"Warned {user.name}#{user.discriminator}.")
            await ctx.send(embed=embed, delete_after=7)
        else:
            embed = discord.Embed(color = discord.Colour.green(), description = f"Warned {user.name}#{user.discriminator}. Reason: {reason}")
            await ctx.send(embed=embed, delete_after=7)
            ...
            try:
                ...
                async with db.execute("SELECT reason FROM warnings WHERE guild = ? and member_id = ?", (guildid,member)) as cursor:
                    ...
                    if number > 0:
                        ...
                        embed = discord.Embed(color = discord.Colour.red(), description = f"{user.name}#{user.discriminator} has no warnings.")
                        await ctx.send(embed=embed, delete_after=5)
                await db.close()
            except:
                embed = discord.Embed(color = discord.Colour.red(), description = f"{user.name}#{user.discriminator} has no warnings.")
    ...
    async def unwarn(ctx, member, dir, number):
        ...
        if number == "all":
            ...
            await db.commit()
            await db.close()
            embed = discord.Embed(color = discord.Colour.green(), description = f"Cleared all warnings from {user.name}#{user.discriminator}.")
            await ctx.send(embed=embed, delete_after=7)
            return
        else:
                try:
                    if "," in number:
                        ...
                        embed = discord.Embed(color = discord.Colour.green(), description = f"Cleared warnings {number_list} from .{user.name}#{user.discriminator}.")
                        await ctx.send(embed=embed, delete_after=7)
                    else:
                        number = int(number)
                        dict = {}
                        ...
                        await db.commit()
                        await db.close()
                        embed = discord.Embed(color = discord.Colour.green(), description = f"Cleared {user.name}#{user.discriminator}'s' #{number} warning.")
                        await ctx.send(embed=embed, delete_after=7)
                except:
                    if number == "all":
                        embed = discord.Embed(color = discord.Colour.red(), description = f"{user.name}#{user.discriminator} has no warnings.")
                        ...
                        embed = discord.Embed(color = discord.Colour.red(), description = f"{user.name}#{user.discriminator} does not have that many warnings.")
                        await ctx.send(embed=embed, delete_after=5)
...
class rr():
    async def command(ctx, emoji, dir, role, *args, title, description, **kwargs):
        ...
        if "," in emoji:
            ...
            for x in emoji_list:
                ...
                if "@" not in role:
                    embed = discord.Embed(color = discord.Colour.red(), description = "Invalid Role. Please read https://sites.google.com/view/dpys/reaction-roles")
    ...
    async def clear_all(ctx, dir):
        ...
        await db.commit()
        await db.close()
        embed = discord.Embed(color = discord.Colour.green(), description = "Deleted all reaction role info for this server.")
        await ctx.send(embed=embed, delete_after=7)
    ...
    async def clear_one(ctx, dir, message_id):
        ...
        embed = discord.Embed(color = discord.Colour.green(), description = f"Delted reaction role info for {message_id}")
        await ctx.send(embed=embed, delete_after=7)
    ...
    async def display(ctx, dir):
        ...
        if number > 0:
            if limit == "limit":
                ...
                embed.set_footer(text=f"Total Reaction Roles | {number}")
                limit_embed = discord.Embed(color = discord.Colour.red(), description = "One of your reaction roles went over the Discord limit. It will still work perfectly but only essential data will be displayed in this command to save space.")
                await ctx.reply(embed=limit_embed, delete_after=7)
            await ctx.send(embed=embed, reference= ctx.message.reference or ctx.message)
        else:
            embed = discord.Embed(color = discord.Colour.red(), description = "There are no reaction roles in this server.")
            await ctx.send(embed=embed, delete_after=5)
        await db.close()
