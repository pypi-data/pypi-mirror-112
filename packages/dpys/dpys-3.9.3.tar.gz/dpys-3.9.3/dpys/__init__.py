...
class curse():
    async def add_banned_word(ctx, word, dir):
        ...
        if number == 0:
            if len(words) > 1:
                await ctx.send("Added those words to the list.", delete_after=7)
            else:
                await ctx.send("Added that word to the list.", delete_after=7)
        else:
            if number != len(words_unfiltered):
                if len(words) > 1:
                    await ctx.send(f"Added {len(words)}/{len(words_unfiltered)} to the list. The others were already in the list.", delete_after=7)
                else:
                    await ctx.send("That word is already in the list,", delete_after=7)
    async def remove_banned_word(ctx, word, dir):
            ...
            try:
                ...
                await db.close()
                if number != len(word):
                    await ctx.send(f"Only {number}/{len(word)} of those words were in the list. They have been successfully removed.", delete_after=7)
                else:
                    if len(word) == 1:
                        await ctx.send("Successfully removed that word from the list.", delete_after=7)
                    else:
                        await ctx.send("Successfully removed those words from the list.", delete_after=7)
            except:
