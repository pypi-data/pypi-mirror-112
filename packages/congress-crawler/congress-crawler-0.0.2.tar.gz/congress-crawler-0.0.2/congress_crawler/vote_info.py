...
def fetch_vote(vote_id, options):
    ...
    body = utils.download(
        url,
        f"{vote_congress}/votes/{vote_session_year}/{vote_chamber}{vote_number}/{vote_chamber}{vote_number}.xml",
...
def parse_senate_vote(dom, vote):
    ...
    def add_vote(vote_option, voter):
        ...
        if voter != "VP" and voter["id"] == "":
            ...
            if voter["id"] == None:
                logging.error(
                    "[{}] Missing lis_member_id and name lookup failed for {}".format(
                        vote["vote_id"], voter["last_name"]
                    )
                )
                raise Exception(
                    "Could not find ID for {} ({}-{})".format(
                        voter["last_name"], voter["state"], voter["party"]
                    )
                )
            else:
                logging.info(
                    "[{}] Missing lis_member_id, falling back to name lookup for {}".format(
                        vote["vote_id"], voter["last_name"]
                    )
                )
...
def parse_house_vote(dom, vote):
    ...
    if "amendment" in vote and "bill" in vote:
        vote["question"] += ": Amendment {} to {}".format(
    ...
    for v in all_voters:
        ...
        if v["id"] == None:
            logging.error(
                "[{}] Missing bioguide ID and name lookup failed for {} ({}-{} on {})".format(
                    vote["vote_id"], display_name, v["state"], v["party"], vote["date"]
                )
            )
            raise Exception(
                "No bioguide ID for {} ({}-{})".format(
            ...
            if vote["congress"] > 107:
                logging.warning(
                    "[{}] Used name lookup for {} because bioguide ID was missing.".format(
...
def get_vote_category(vote_question):
    ...
    # unhandled
    logging.warning("Unhandled vote question: %s" % vote_question)
    return "unknown"
