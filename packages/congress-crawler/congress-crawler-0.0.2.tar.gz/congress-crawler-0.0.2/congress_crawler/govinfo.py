...
def update_sitemap2(
    ...
    try:
        ...
        raise Exception(f"XML syntax error in {url}: {str(e)}")
    ...
    if sitemap.tag == "{http://www.sitemaps.org/schemas/sitemap/0.9}sitemapindex":
        ...
        for node in sitemap.xpath("x:url", namespaces=ns):
            ...
            if m:
                ...
                try:
                    ...
                    logging.exception(
                        f"Error fetching package {package_name} in collection {collection} from {url}."
                    )
                    mirror_results = []
                ...
                if not m:
                    raise Exception(
                        "Unmatched bulk data file URL ({}) at {}.".format(
                ...
                try:
                    ...
                    logging.exception(
                        f"Error fetching file {item_path} in collection {collection} from {url}."
                    )
                    mirror_results = None
        ...
        raise Exception(
            f"Unknown sitemap type ({sitemap.tag}) at the root sitemap of {url}."
        )
...
def mirror_package_zipfile(
    ...
    # Download.
    file_url = GOVINFO_BASE_URL + f"content/pkg/{collection}-{package_name}.zip"
    logging.warning("Downloading: " + file_path)
    data = utils.download(
        file_url,
        file_path,
        utils.merge(
...
def extract_package_files(
    ...
    with zipfile.ZipFile(package_file) as package:
        for format in extract_formats:
            ...
            logging.warning("Extracted: " + local_path)
...
def get_output_path(collection, package_name, options):
    ...
    if collection == "BILLS":
        ...
        if options.get("congress") and congress != options.get("congress"):
            return None  # congress number does not match options["congress"]
        return f"{utils.data_dir()}/{congress}/{collection.lower()}/{report_type}/{report_type + report_number}"

    else:
        # Store in govinfo/COLLECTION/PKGNAME.
        path = f"{utils.data_dir()}/govinfo/{collection}/{package_name}"
        return path
...
def mirror_bulkdata_file(collection, url, item_path, lastmod, options):
    ...
    path = f"{utils.data_dir()}/govinfo/{collection}/{item_path}"
    ...
    # Download.
    logging.warning("Downloading: " + path)
    data = utils.download(
        url,
        path,
        utils.merge(
...
def output_for_bill_version(bill_version_id):
    ...
    return f"{utils.data_dir()}/{congress}/bills/{bill_type}/{bill_type}{number}/text-versions/{version_code}/data.json"
