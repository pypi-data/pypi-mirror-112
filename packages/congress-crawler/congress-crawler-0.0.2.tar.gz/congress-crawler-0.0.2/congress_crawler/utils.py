...
def build_bill_version_id(bill_type, bill_number, congress, version_code):
    return f"{bill_type}{bill_number}-{congress}-{version_code}"
...
def split_nomination_id(nomination_id):
    try:
        ...
        logging.error(f"Unable to parse {nomination_id}")
    ...
    for id in to_fetch:
        ...
        if results.get("ok", False):
            if results.get("saved", False):
                ...
                skips.append(id)
                logging.warning("[{}] Skipping: {}".format(id, results["reason"]))
        else:
            errors.append((id, results, None))
            logging.error("[{}] Error: {}".format(id, results["reason"]))

    if len(errors) > 0:
        message = f"\nErrors for {len(errors)} items:\n"
        for _id, error, msg in errors:
            message += "\n\n"
            if isinstance(error, Exception):
                message += f"[{_id}] Exception:\n\n"
                message += msg
            else:
                message += f"[{_id}] {error}"
...
def download(url, destination=None, options: dict = None):
    if not options:
        options = {}
    ...
    if destination and to_cache:
        ...
        for i in range(len(dparts) - 1):
            ...
            if not test:
                logging.info("Cached: ({}, {})".format(zfn + "#" + zfn_inner, url))
            if force:
                raise Exception(
    ...
    if destination and (not force) and os.path.exists(cache_path):
        if not test:
            logging.info(f"Cached: ({cache_path}, {url})")
...
def write(content, destination, options: dict = None):
    if options is None:
        options = {}
    if options.get("diff"):
        ...
        with open(fn, "w") as f:
            f.write(content)
        os.system(f"diff -u {destination} {fn}")
        os.unlink(fn)
        return
...
def xpath_regex(doc, element, pattern):
    return doc.xpath(
        f"//{element}[re:match(text(), '{pattern}')]",
...
def unescape(text):
    ...
    def fixup(m):
        ...
        if _text[:2] == "&#":
            ...
            # named entity
            try:
                _text = chr(html.entities.name2codepoint[text[1:-1]])
            except KeyError:
                pass
...
def extract_bills(text, session):
    ...
    if bill_matches:
        for b in bill_matches:
            bill_text = (
                f"{b[0].lower().replace(' ', '').replace('.', '').replace('con', 'c')}"
                f"-{session}"
            )
...
def cache_load(cache_filename, file_hash):
    ...
    if cache_data["hash"] != file_hash:
        raise CacheError(
            "Hashes do not match: {}, {}".format(file_hash, cache_data["hash"])
        )
...
def yaml_load(filename):
    ...
    try:
        ...
        logging.warning("Using original YAML file...")
...
def require_congress_legislators_repo():
    ...
    if not os.path.exists("congress-legislators"):
        logging.warning("Cloning the congress-legislators repo...")
        os.system(
    ...
    if os.environ.get("UPDATE_CONGRESS_LEGISLATORS") != "NO":
        ...
        logging.warning("Updating the congress-legislators repo...")
...
def lookup_legislator(
    congress, role_type, name, state, party, when, id_requested, exclude=None
):
    ...
    def to_ascii(_name):
        ...
        return "".join(
            x
            for x in unicodedata.normalize("NFKD", _name)
    ...
    if len(matches) == 0:
        logging.warning(
            f"Could not match name {name} ({state}-{party}; {when}) to any legislator."
        )
        return None
    if len(matches) > 1:
        logging.warning(
            f"Multiple matches of name {name} ({state}-{party}; {when}) to legislators ({str(matches)}; excludes {str(exclude)})."
        )
        return None
...
class UnmatchedIdentifer(Exception):
    def __init__(self, id_type, id_value, desired_id_type):
        super().__init__(f"{id_type}={str(id_value)} => {desired_id_type}")
...
def translate_legislator_id(source_id_type, source_id, dest_id_type):
    ...
    if not _translate_legislator_id_cache:
        ...
        for filename in ("legislators-historical", "legislators-current"):
            for moc in yaml_load(f"congress-legislators/{filename}.yaml"):
    ...
    def __enter__(self):
        ...
        for sig in self.sigs:
            ...
            def handler(
                ...
                self.signal_received[_sig] = (s, frame)
                # Note: in Python 3.5, you can use signal.Signals(sig).name
                logging.info("Signal %s received. Delaying KeyboardInterrupt." % _sig)
