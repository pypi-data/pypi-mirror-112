...
from congress_crawler import amendment_info, bill_info, govinfo, utils
...
def run(options):
    ...
    if bill_id:
        ...
        if not to_fetch:
            logging.warning("No bills changed.")
            return None
...
def process_bill(bill_id, options):
    ...
    logging.info(f"[{bill_id}] Processing {fdsys_xml_path}...")
...
def output_for_bill(bill_id, format, is_data_dot=True):
    ...
    if is_data_dot:
        ...
        fn = format
    return f"{utils.data_dir()}/{congress}/bills/{bill_type}/{bill_type}{number}/{fn}"
...
def reparse_actions(bill_id, options):
    ...
    with open(data_xml_fn) as xml_file:
