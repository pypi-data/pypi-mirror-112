...
def run(options):
    ...
    for c in utils.yaml_load("congress-legislators/committees-current.yaml"):
        ...
        c["subcommittees"] = {s["thomas_id"]: s for s in c.get("subcommittees", [])}
...
def fetch_senate_committee_meetings(committees, options):
    ...
    for node in dom.xpath("meeting"):
        ...
        if options.get("debug", False):
            print(
                f"[senate][{committee_code}][{subcommittee_code}] Found meeting in room {room} at {occurs_at.isoformat()}."
            )
...
def parse_house_committee_meeting(
    ...
    for committee_code, subcommittee_code in orgs:
        ...
        if options.get("debug", False):
            print(
                f"[house][{committee_code}][{subcommittee_code}] Found meeting in room {room} at {occurs_at.isoformat()}"
            )
...
def save_documents(package, event_id):
    ...
    output_dir = utils.data_dir() + f"/committee/meetings/house/{folder}/{event_id}"
...
def text_from_pdf(pdf_path):
    try:
        ...
        logging.warning("Not processing PDF. Have you installed pdftotext?")
        return None
    ...
    try:
        subprocess.check_call(
            f'pdftotext -layout "{real_pdf_path}" "{real_text_path}"', shell=True
        )
    except subprocess.CalledProcessError as exc:
        logging.warning("Error extracting text for %s\n" % (real_text_path))
        return None
    ...
    if os.path.exists(real_text_path):
        ...
        logging.warning("Text not extracted to %s" % real_text_path)
        return None
