...
from grid.tar import get_split_size, tar_path
from grid.uploader import S3Uploader, UploadProgressCallback
...
class DatastoreRemoteUpload:
    ...
    def upload(self):
        ...
        try:
            create_datastore(
                name=self.name, credential_id=self.credential_id, client=self.client, source=self.source_url
            )
...
class DatastoreUploadSession(UploadProgressCallback):
    ...
    def _create_datastore(self):
        ...
        datastore_id, datastore_version = create_datastore(
            name=self.name, credential_id=self.credential_id, client=self.client, source=self.uploaded_target_file
        )
