...
def send_to_redshift(database: str, schema: str, table: str, df: pd.DataFrame, **kwargs: Any) -> None:
    """
        ...
        Use this function to send data to redshift and s3 bucket.
        ...
        Parameters:
              ...
              check(bool): False by default. If True, check the column type and length consistency before sending data.
              drop(bool): False by default. If True, drop the datatable which exists already.
        Examples:
            >>> import os
            >>> import rcd_pyutils
            >>> rcd_pyutils.send_to_redshift(database="my_database", schema="my_schema", table="my_table", df=my_dataframe)
    """
    so = S3Operator()
    so.bucket = kwargs.get("bucket", os.environ.get("S3_BUCKET_DATAMART"))
    so.send_to_s3_obj(df=df, s3_file_path=os.path.join(schema, f"{table}.csv"))
    ...
    if kwargs.get("drop") is True:
        ro.drop_table()
    ro.process_table(bucket=kwargs.get("bucket", os.environ.get("S3_BUCKET_DATAMART")), df=df,
...
def read_from_redshift(database: str, method: str, **kwargs) -> pd.DataFrame:
    """
        ...
        Examples:
            >>> import os
            >>> import rcd_pyutils
            >>> rcd_pyutils.read_from_redshift(database="my_database", method="auto", schema="my_schema", table="my_table")
            >>> rcd_pyutils.read_from_redshift(database="my_database", method="sql", sql_query='SELECT * FROM my_schema.my_table')
    """
    ro = RedshiftOperator(database=database)
...
class RedshiftOperator:
    ...
    Examples:
        >>> import rcd_pyutils
        >>> ro = rcd_pyutils.RedshiftOperator()
        >>> ro.read_from_redshift(schema="my_schema", table="my_table", limit=10)
    """

    def __init__(self, database: str = os.environ.get("REDSHIFT_DB")) -> None:
    ...
    def copy_from_s3(self, bucket: str, prefix: str, s3_table: str, redshift_schema: str, redshift_table: str) -> None:
        sep = "|"
    ...
    def process_table(self, bucket: str, df: pd.DataFrame, check: bool = False) -> None:
        ...
        self.copy_from_s3(bucket=bucket, prefix=self._schema, s3_table=self._table, redshift_schema=self._schema,
