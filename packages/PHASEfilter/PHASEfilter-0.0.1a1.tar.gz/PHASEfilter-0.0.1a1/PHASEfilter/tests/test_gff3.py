...
class Test(unittest.TestCase):
	...
	def test_gff3_2(self):
		...
		lift_over_ligth = LiftOverLight(reference_a, reference_b, temp_work_dir, impose_minimap2_only, True)
		lift_over_ligth.synchronize_sequences(seq_name_a, seq_name_b)
		self.assertEqual((50036, -1), lift_over_ligth.get_pos_in_target(seq_name_a, seq_name_b, position))
		...
		#####
		file_gff = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'files/gff3/to_translate.gff')
		self.assertTrue(os.path.exists(file_gff))
		
		read_gff = ReadGFF(file_gff)
		file_result_gff = utils.get_temp_file("file_result_gff", ".gff3")
		vect_type_to_process = [ReadGFF.PROCESS_TYPE_all]
		read_gff.parse_gff(file_result_gff, vect_type_to_process, [], lift_over_ligth)
		self.assertEqual(Software.SOFTWARE_blast_name, lift_over_ligth.get_best_algorithm('chrXVI', 'chrXVI'))
		
		file_expected_gff = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'files/gff3/file_result_translate_gff.gff3')
		self.assertTrue(os.path.exists(file_expected_gff))
		
		cmd = "diff {} {} > {}".format(file_result_gff, file_expected_gff, temp_diff)
		os.system(cmd)
		vect_result = utils.read_text_file(temp_diff)
		self.assertEqual(0, len(vect_result))
		
		utils.remove_file(file_result_gff)
