...
@app.route('/createKML', methods=[restMethods.POST])
@auth.login_required()
def create_kml():
    ...
    root.Folder.Placemark[0].append(KML.Point(KML.coordinates("0,0")))
    ...
    data_stringified = etree.tostring(root)
    with open(filepath, mode="wb+") as file:
        ...
        with ZipFile(file, mode='a') as zip:
            ...
            if "MANIFEST/manifest.xml" not in [member.filename for member in zip.infolist()]:
                ...
                SubElement(config, "Parameter", name="name", value=name+".zip")
                zip.writestr(str(hashlib.sha256(data_stringified).hexdigest()) + "/" + name + ".kml", data_stringified)
    ...
    uid = str(uuid.uuid4())
    dbController.create_datapackage(uid=uid, Name=name+".zip", Hash=file_hash, SubmissionUser='server',
...
def broadcast_datapackage(uid):
    ...
    clientXML = f'<?xml version="1.0"?><event version="2.0" uid="{str(uuid.uuid4())}" type="b-f-t-r" time="{time}" start="{time}" stale="{stale}" how="h-e"><point lat="43.85570300" lon="-66.10801200" hae="19.55866360" ce="3.21600008" le="nan" /><detail><fileshare filename="{DPObj.Name}" senderUrl="{DPIP}:8080/Marti/api/sync/metadata/{DPObj.Hash}/tool" sizeInBytes="{DPObj.Size}" sha256="{str(DPObj.Hash)}" senderUid="{"server-uid"}" senderCallsign="{"server"}" name="{DPObj.Name + ".zip"}" /><ackrequest uid="{uuid.uuid4()}" ackrequested="true" tag="{DPObj.Name + ".zip"}" /></detail></event>'
    cot.xmlString = clientXML.encode()
    newCoT = SendOtherController(cot, addToDB=False)
    APIPipe.put(newCoT.getObject())
