...
def create_parser():
    ...
    required.add_argument('-e', '--edges', metavar = '', help = 'comma-separated names of edges in the assembly graph to be mapped', required = True)
...
def gfa_to_fasta(gfa, edges, prefix, verbose):
    ...
    for entry in file:
        entry = entry.split()
        if entry[0] == 'S' and entry[1] in edges:
            if verbose:
