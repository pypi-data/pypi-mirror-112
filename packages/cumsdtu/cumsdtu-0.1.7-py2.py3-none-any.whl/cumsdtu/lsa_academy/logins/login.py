...
class Login(BaseResposne):
    ...
    INSIGNIFICANT_KEYS=['extraValueMap','helperData','nextStageId','paymentDescription','saveDeleteRecord','sessionId','showSpecializations',
                        'maximumRegistrationCredit','minimumRegistrationCredit','registrationNo','registrationRoundNo',
                        'specialization1','specialization2','status','totalAmount','creditsRegistered','electiveCoursesList',
                        'remainingCredit','remainingCourses', 'choiceType', 'countVisible', 'credits', 'creditsTypeChoice', 'extraInfo',
    ...
    def extract_info(self,**kwargs):
        ...
        if self.file_name is not None:
            ...
            return self.custom_response(False,"Course details written to {} successfully".format(self.file_name),200,extra_info=self.reg_data.get('data').get('extraInfo'))
        ...
        return self.custom_response(False,"Course Details retrived",200,**{'reg_data':self.reg_data.get('data'),'course_detail':self.course_detail,**kwargs})
...
def test():
    ...
    return Login.do_login(applicant,persistor=persistor,show_courses=True)
