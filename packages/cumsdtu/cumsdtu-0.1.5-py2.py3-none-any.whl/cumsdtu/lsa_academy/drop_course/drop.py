...
class DropCourse(Login):
    ...
    def __init__(self,to_verify,**kwargs):
        ...
        self.persistor=self.safe_dict(kwargs,'persistor',False)
    ...
    def query_info(self, **kwargs):
        ...
        if not json_data.get('error'):
            ...
            return self.custom_response(json_data.get('error'),json_data.get('msgLst')[-1].get('errCode'),409)
...
def test():
    ...
    return DropCourse.extract_data(applicant, persistor=persistor)
