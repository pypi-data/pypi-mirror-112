...
class VoiceTextContext(c.Context):
    ...
    async def join(self) -> None:
        ...
        print(f"start: {0}".format("\n".join(f"{k}: {v}" for k, v in self.default_params.items())))
        if not await self.bot_channels.fetch(**self.default_params):
            await self.bot_channels.insert(**self.default_params)
            print(f"insert: {0}".format("\n".join(f"{k}: {v}" for k, v in self.default_params.items())))

        await self.bot_channels.update(tc=self.channel.id, **self.default_params)
        print(f"update: tc: {self.channel.id}")
