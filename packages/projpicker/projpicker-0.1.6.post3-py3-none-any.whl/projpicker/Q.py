...
def projpicker(
    ...
    Args:
        ...
        geom_per_line (bool): Whether or not to read a geometry per line. If
            this argument is True, coordinates in a geometry can be separated
            by whitespaces, but only one geometry is allowed per line.
            Otherwise, by default, only commas can be used to separate
            coordinates in a geometry, but multiple geometries are allowed in a
            line. In this case, any whitespaces in units should be enclosed by
            single or double quotes. Defaults to False.
...
def parse():
    ...
    parser.add_argument(
            "-l", "--geometry-per-line",
            action="store_true",
            help="read a geometry per line; with this option, coordinates in "
                "a geometry can be separated by whitespaces, but only one "
                "geometry is allowed per line; without this option by "
                "default, only commas can be used to separate coordinates in "
                "a geometry, but multiple geometries are allowed in a line")
    parser.add_argument(
            "-p", "--print-geometries",
            action="store_true",
...
def main():
    ...
    geom_per_line = args.geometry_per_line
