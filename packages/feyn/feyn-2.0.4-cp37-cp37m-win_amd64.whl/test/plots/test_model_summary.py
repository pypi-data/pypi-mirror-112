...
class TestModelSummary(unittest.TestCase):
    ...
    def test_validate_data_inputs(self):
        with self.subTest("Disallows tomfoolery"):
            df = ['Hello world', pd.DataFrame()]
            with self.assertRaises(ValueError):
                actual = _sanitize_data_inputs(df)

        with self.subTest("Allows single dataframe inputs"):
            df = pd.DataFrame()
            actual = _sanitize_data_inputs(df)
            self.assertEqual([df], actual)

        with self.subTest("Allows list of dataframe inputs"):
            df = [pd.DataFrame()]
            actual = _sanitize_data_inputs(df)
            self.assertEqual(df, actual)
