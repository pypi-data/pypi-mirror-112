...
class DialogHandler(logging.Handler):
  weakWin: weakref.ReferenceType = None
...
class StatusBarHandler(DialogHandler):

  def __init__(self, *args, defaultMsgtimeout=3000, **kwargs) -> None:
    super().__init__(*args, **kwargs)
    self.defaultMsgTimeout = defaultMsgtimeout
  ...
  def emit(self, record: logging.LogRecord):
    ...
    win.statusBar().showMessage(self.format(record), msgTimeout)

  def filter(self, record: logging.LogRecord):
    return record.levelno < PrjEnums.LOG_LVL_ATTN and logging.Handler.filter(self, record)