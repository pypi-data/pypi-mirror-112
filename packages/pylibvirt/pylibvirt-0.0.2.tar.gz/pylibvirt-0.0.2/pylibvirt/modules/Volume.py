...
def create_disk(storage: libvirt.virStoragePool, capacity: int, name: str,
    ...
    new_volume = Volume(name=name, capacity=capacity, disk_format=disk_format, size_unit=size_unit,
                        allocation=allocation)
    return storage.createXML(new_volume.generate_xml(), 0)


def clone_disk(storage: libvirt.virStoragePool, volume: libvirt.virStorageVol, capacity: int, name: str,
               disk_format: str = 'raw', size_unit: str = 'G', allocation: int = 0):
    new_volume = Volume(name=name, capacity=capacity, disk_format=disk_format, size_unit=size_unit,
                        allocation=allocation)
...
class Volume(Device):
    ...
    def __init__(self, name: str, capacity: int, allocation: int = 0, size_unit: str = 'G', disk_format: str = "raw"):
