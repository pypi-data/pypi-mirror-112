...
class dBar:
    ...
    def __init__(self, k_grid, Now, Ref, R_z, m_z):
        ...
        self.Z = np.zeros(int(pow(2, 2*m)), dtype=complex)
        self.load_mesh(1, m_z)
        self.sigma(self.Z.size)
    ...
    def solve(self, k_grid):
        ...
        for i in range(1, self.Z.size):
                ...
                if(abs(zz) <= 1):
                    self.sigma[i] = mu[indx]*mu[indx]-mu[indx+N]*mu[indx+N]


    def plot(self):
        
        Z_N = math.sqrt(self.Z.size)
        h = 2/(Z_N)
        for i in range(Z_N):
            X[i] = - 1 + i*h
            Y[i] = - 1 + i*h
        ...
        for i in range(Z_N):
            if i%2==0:
                for j in range(Z_N):
                    sigma_x[i, j] = sigma[j+i*Z_N]
            else:
                for j in range(Z_N):
                    sigma_x[i, j] = sigma[(Z_N-1-j)+i*Z_N]
