...
class OC(SSH):
    ...
    @typechecked
    @logger_time_stamp
    def get_pod_name(self, pod_name: str, namespace: str):
        """
    ...
    @typechecked
    @logger_time_stamp
    def get_vmi_name(self, vm_name: str, namespace: str):
        """
        This method return pod name if exist or empty string
        :return:
        """
    ...
    def login(self):
        ...
        """
        try:
            return self.run(f'~/./oc login -u kubeadmin -p $(cat ~/.kube/kubeadmin-password)')
        except Exception as err:
            raise LoginFailed
    ...
    def wait_for_pod_create(self, pod_name: str, namespace: str = "my-ripsaw", timeout: int = 300, sleep_time: int = 10):
        ...
        while current_wait_time <= timeout:
            if self.get_pod_name(pod_name, namespace):
                return True
            # sleep for x seconds
    ...
    def wait_for_vm_create(self, vm_name: str, namespace: str = "my-ripsaw", timeout: int = 300, sleep_time: int = 10):
        ...
        current_wait_time = 0
        while current_wait_time <= timeout:
            if self.get_vmi_name(vm_name, namespace):
                return True
            # sleep for x seconds
            time.sleep(sleep_time)
            current_wait_time += sleep_time
    ...
    def wait_for_pod_terminate(self, pod_name: str, namespace: str = "my-ripsaw", timeout: int = 300, sleep_time: int = 10):
        ...
        current_wait_time = 0
        while current_wait_time <= timeout:
            if not self.get_pod_name(pod_name, namespace):
                return True
            # sleep for x seconds
            time.sleep(sleep_time)
            current_wait_time += sleep_time
    ...
    def wait_for_vm_terminate(self, vm_name: str, namespace: str = "my-ripsaw", timeout: int = 300, sleep_time: int = 10):
        ...
        current_wait_time = 0
        while current_wait_time <= timeout:
            if not self.get_vmi_name(vm_name, namespace):
                return True
            # sleep for x seconds
            time.sleep(sleep_time)
            current_wait_time += sleep_time
