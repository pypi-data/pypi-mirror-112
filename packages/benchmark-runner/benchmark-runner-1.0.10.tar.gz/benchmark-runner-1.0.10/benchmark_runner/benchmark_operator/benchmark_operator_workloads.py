...
class BenchmarkOperatorWorkloads:
    ...
    def __init__(self, es_host: str = ''):
        ...
        if es_host:
            self.__es_operations = ESOperations(es_host=es_host, es_port=9200)
        # Generate templates class
        self.template = TemplateOperations()
        self.__oc.login()
    ...
    def helm_install_benchmark_operator(self):
        ...
        if not self.__oc.get_pod_name(pod_name='benchmark-operator', namespace='my-ripsaw'):
    ...
    def run_workload_func(self, workload_full_name: str):
        ...
        if 'hammerdb' in workload_full_name:
            self.template.generate_hammerdb_yamls(workload=f'{workload_name[0]}_{workload_name[1]}', database=workload_name[2])
            class_method = getattr(BenchmarkOperatorWorkloads, f'{workload_name[0]}_{workload_name[1]}')
            class_method(self, workload_name[2])
        else:
            self.template.generate_workload_yamls(workload=workload_full_name)
