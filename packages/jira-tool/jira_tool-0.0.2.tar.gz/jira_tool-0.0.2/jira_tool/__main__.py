...
def get_labels_from_markdown(url, file):
    fo = open(url + '/' + file, "r")
...
def get_title_from_markdown(url, file):
    fo = open(url + '/' + file, "r")
    fo.readline()
...
def creat_jira_card(jira_desc, labels, title):
    url = "https://itsc-jira.daimler.com/jira/rest/api/2/issue/bulk"
    ...
    response = requests.request("POST", url, auth=HTTPBasicAuth(config['name'], config['password']), headers=headers,
                                data=payload)
...
def get_card_number_from_response(response):
    response_json = json.loads(response)
    return response_json["issues"][0]["key"]


def rename_file_to_card_number(url, file_name, card_number):
    os.rename(url + "/" + file_name, url + "/" + card_number + ".md")
    pass


def create_jira_card(card_no, file_url):
    file_name = card_no + ".md"
    print(card_no)
    labels = get_labels_from_markdown(file_url, file_name)
    print(labels)
    jira_title = get_title_from_markdown(file_url, file_name)
    print(jira_title)
    jira_desc = get_markdown_info_with_jira_format(file_url, file_name)
    print(jira_desc)
    response = creat_jira_card(jira_desc, labels, jira_title)
    card_no = get_card_number_from_response(response)
    rename_file_to_card_number(file_url, file_name, card_no)
...
def main(args=None):
    ...
    if len(sys.argv) == 3:
        card_no = sys.argv[2]
        create_jira_card(card_no, file_url)
        return 0
    else:
