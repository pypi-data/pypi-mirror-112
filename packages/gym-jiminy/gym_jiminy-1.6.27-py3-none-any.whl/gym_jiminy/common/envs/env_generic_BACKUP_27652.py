...
class BaseJiminyEnv(ObserverControllerInterface, gym.Env):
    ...
    def play_interactive(self,
        ...
        def _interact(key: Optional[str] = None) -> bool:
            action = self._key_to_action(key, **kwargs)
...
<<<<<<< HEAD
        # Enable travelling
        if self.simulator.viewer.backend.startswith('panda3d'):
        ...
        if enable_travelling:
            ...
            self.simulator.viewer.attach_camera(tracked_frame, camera_xyzrpy)

        # Run interactive loop
        loop_interactive(max_rate=self.step_dt, verbose=True)(_interact)()

        # Disable travelling if it enabled
        if enable_travelling:
            self.simulator.viewer.detach_camera()
=======
        # Enable position-only travelling
        if self.robot.has_freeflyer:
            self.simulator.viewer.attach_camera(
                self.robot.pinocchio_model.frames[2].name, None)

        loop_interactive(max_rate=self.step_dt, verbose=True)(_interact)
>>>>>>> 516873ce... [python/viewer] Add option to not lock camera pose when travelling is enabled using Panda3d.
    ...
    def _key_to_action(self,
        ...
        .. note::
            The mapping can be state dependent, and the key used for something
            different than computing the action directly. For instance, one can
            provide a custom policy having user parameters in input as extra
            argument to this method.

        .. warning::
            This method is called before `step` method systematically, even if
            not key has been pressed. In such a case the key is `None`.
