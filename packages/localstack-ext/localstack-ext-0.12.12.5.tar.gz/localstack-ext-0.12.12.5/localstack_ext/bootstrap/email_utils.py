...
SENT_EMAILS=[]
EMAIL_BLACKLIST=VaDUr()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r'\s+','',VaDUn(email or ''))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=VaDUu,from_name=VaDUu,smtp_host=VaDUu,smtp_user=VaDUu,smtp_pass=VaDUu,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if VaDUw(recipients,VaDUl)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=VaDUu):
 ...
 result['From']=formataddr((VaDUn(Header(from_name,'utf-8')),from_email))
 return result
