...
SENT_EMAILS=[]
EMAIL_BLACKLIST=sWISq()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r'\s+','',sWISV(email or ''))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=sWISt,from_name=sWISt,smtp_host=sWISt,smtp_user=sWISt,smtp_pass=sWISt,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if sWISz(recipients,sWISu)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=sWISt):
 ...
 result['From']=formataddr((sWISV(Header(from_name,'utf-8')),from_email))
 return result
