...
SENT_EMAILS=[]
EMAIL_BLACKLIST=BqWlC()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r'\s+','',BqWlK(email or ''))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=BqWlH,from_name=BqWlH,smtp_host=BqWlH,smtp_user=BqWlH,smtp_pass=BqWlH,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if BqWln(recipients,BqWlR)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=BqWlH):
 ...
 result['From']=formataddr((BqWlK(Header(from_name,'utf-8')),from_email))
 return result
