...
SENT_EMAILS=[]
EMAIL_BLACKLIST=eSQwy()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r'\s+','',eSQwA(email or ''))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=eSQwN,from_name=eSQwN,smtp_host=eSQwN,smtp_user=eSQwN,smtp_pass=eSQwN,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if eSQwc(recipients,eSQwn)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=eSQwN):
 ...
 result['From']=formataddr((eSQwA(Header(from_name,'utf-8')),from_email))
 return result
