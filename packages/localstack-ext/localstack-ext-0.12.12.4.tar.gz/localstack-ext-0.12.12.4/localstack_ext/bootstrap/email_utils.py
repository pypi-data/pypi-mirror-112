...
SENT_EMAILS=[]
EMAIL_BLACKLIST=ORgrL()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r'\s+','',ORgrV(email or ''))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=ORgrT,from_name=ORgrT,smtp_host=ORgrT,smtp_user=ORgrT,smtp_pass=ORgrT,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if ORgrw(recipients,ORgrC)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=ORgrT):
 ...
 result['From']=formataddr((ORgrV(Header(from_name,'utf-8')),from_email))
 return result
