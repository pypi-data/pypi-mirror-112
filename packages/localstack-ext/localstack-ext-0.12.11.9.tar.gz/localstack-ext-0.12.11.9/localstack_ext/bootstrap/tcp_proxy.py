...
class AttrDict(MLyri):
 def __init__(self,*args,**kwargs):
  MLyrF(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and MLyrE(socket,'IPV6_V6ONLY'):
...
class Half(MLyrV):
 def __init__(self,opt,sock,addr,MLyrf):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.MLyrf=MLyrf
  self.name='peer' if self.MLyrf else 'client'
  self.queue=[]
  self.dest=MLyrq
  self.err=MLyrq
  self.ready=MLyrT
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':MLyrT,'bind_port':2223,'addr':'192.168.43.191','port':22}
 opt={'ip6':MLyrT,'bind_port':48429,'addr':'128.0.100.3','port':22}
 opt={'ip6':MLyrT,'port':54237,'bind_addr':'128.0.100.2','bind_port':22}
 opt={'ip6':MLyrT,'port':54237,'bind_addr':'192.168.100.3','bind_port':22}
 server_loop(opt)
