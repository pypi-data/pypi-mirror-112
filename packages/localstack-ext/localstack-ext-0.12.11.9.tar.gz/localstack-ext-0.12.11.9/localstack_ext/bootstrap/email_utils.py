...
SENT_EMAILS=[]
EMAIL_BLACKLIST=xVzbk()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r'\s+','',xVzbo(email or ''))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=xVzbE,from_name=xVzbE,smtp_host=xVzbE,smtp_user=xVzbE,smtp_pass=xVzbE,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if xVzba(recipients,xVzbm)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=xVzbE):
 ...
 result['From']=formataddr((xVzbo(Header(from_name,'utf-8')),from_email))
 return result
