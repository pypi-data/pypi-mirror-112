...
SENT_EMAILS=[]
EMAIL_BLACKLIST=DcxeM()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r'\s+','',Dcxea(email or ''))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=Dcxef,from_name=Dcxef,smtp_host=Dcxef,smtp_user=Dcxef,smtp_pass=Dcxef,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if Dcxev(recipients,DcxeR)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=Dcxef):
 ...
 result['From']=formataddr((Dcxea(Header(from_name,'utf-8')),from_email))
 return result
