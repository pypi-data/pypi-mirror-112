...
SENT_EMAILS=[]
EMAIL_BLACKLIST=XondL()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r'\s+','',XondI(email or ''))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=XondQ,from_name=XondQ,smtp_host=XondQ,smtp_user=XondQ,smtp_pass=XondQ,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if XondT(recipients,XondU)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=XondQ):
 ...
 result['From']=formataddr((XondI(Header(from_name,'utf-8')),from_email))
 return result
