...
class DataConnection(BaseDataConnection):
    ...
    def read_indices(self, **kwargs):
        ...
        holdout_data_indices_location = training_details['entity']['status']['metrics'][0]['context']['intermediate_model']['schema_location']
        holdout_data_indices_location = '/'.join(holdout_data_indices_location.split('/')[:-3]) + '/holdout_data_indices.tar.gz'
        ...
        if self.type == DataConnectionTypes.S3:
            ...
            try:
                data = self._download_indices_from_cos(cos_client=cos_client, location_path=holdout_data_indices_location)
    ...
    def read(self,
        ...
        if with_holdout_split:
            ...
            if indices is None:
                ...
                x_holdout = data.iloc[indices.values.flatten()].drop(self.auto_pipeline_params['prediction_column'], axis=1)
                y_holdout = data.iloc[indices.values.flatten()][self.auto_pipeline_params['prediction_column']].values
                x = data.loc[set(data.index) - set(indices.values.flatten())].drop(self.auto_pipeline_params['prediction_column'], axis=1)
                y = data.loc[set(data.index) - set(indices.values.flatten())][self.auto_pipeline_params['prediction_column']].values
