...
class Dataset(DataFrame):
    ...
    associated with either a :class:`tecton.interactive.FeatureService` or :class:`tecton.interactive.FeaturePackage`.

    There are 2 types of Datasets: Saved and Logged.

    Saved Datasets are generated manually when calling :meth:`tecton.get_historical_features` by setting `save`
    parameter.

    Logged Datasets are generated automatically when declaring a `FeatureService` with :class:`tecton.LoggingConfig`,
    ...
    def _get_source(self):
        if self._proto.HasField("feature_package_name"):
            return ("Source FeaturePackage", self._proto.feature_package_name)
