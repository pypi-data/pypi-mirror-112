...
class TemporaryCallback:
    ...
    def __enter__(self):
        for component in self.components:
            for state in component.operational_states:
                if state <= self.threshold_state:
                    state.register_callbacks(self.callback_capture, *self.callbacks)
        return self
    ...
    def update_operational_state(self, state: Union[int, str, StateSet]) -> Optional[List]:
        ...
        try:
            if state.has_callbacks:
                self.logger.info(f'executing {len(state.value.callbacks)} callbacks for {state.name}')
            out = state.execute_callbacks()
        finally:
