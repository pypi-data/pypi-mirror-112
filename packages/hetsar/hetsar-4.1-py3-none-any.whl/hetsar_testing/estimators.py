...
class RegressionResultsWrapper:
    ...
    """
    def __init__(self,modelstring,a_x,m_y,m_ys,W,y,id,exog_labels,v_theta_ini,bnds,optim_output,variance,variance_sand,df_est,df_mg_est,N,T,K,robust):
    ...
    def summary_all(self):
        """
        :return summary: (str) Prints individual estimates.
        """
        output = f"""
        SAR model with heterogeneous coefficients \n
        ...
        """
        print(output)
        data = [[f'{self.id}','Param','Est','Std. Err','z','95% CI']]
    ...
    def fitted_values(self):
        """
        Returns fitted value and estimated residuals of the HSAR model.

        :return: (numpy.ndarray, numpy.ndarray) Tuple containing fitted values and estimated residuals
        """
        ...
        return (m_y_hat,m_eps_hat)

def _generate_NT_array(df,id,t,var):
    """
    Takes a pandas.DataFrame and outputs an NxT array of sorted values
    :param df: (pandas.DataFrame)
    :param id: (str) Variable name of the identifier
    :param t: (str) Variable name of the time variable
    :param var: (str) Name of the variable to be transformed.
    :return: (numpy.ndarray) NxT array with values of var sorted by id and t
    """
    N = df[id].nunique()
    T = df[t].nunique()
    return np.array(df.sort_values([id,t])[var]).reshape([N,T])
...
class Hetsar(object):
    ...
    def fit(self,df,y,x,id,t,W,spatial_lags = None,intercept = True, mg_est = False,
            group = None,robust = True):
        ...
        :param id: (str) Name of id variable.
        ...
        :param mg_est: (bool) If True, Mean Group estimates are computed. Defaults to False.
        :param group: (str) Column that provides a group label for every unique value of id. Defaults to None.
        ...
        m_y = _generate_NT_array(df,id,t,y)
        ...
        for xvar in x:
            a_x[:,:,i] = _generate_NT_array(df,id,t,xvar)
            i = i+1
        if type(spatial_lags)==list:
            for xsvar in spatial_lags:
                m_xs = _generate_NT_array(df,id,t,xsvar)
                a_x[:,:,i] = W@m_xs
                i = i+1
        assert(i==K)
        ...
        df_est = format_output(optim_output,N,T,K,variance,variance_sand,y,exog_labels,id)

        if mg_est == True:
            if group== None:
                df_est['group'] = 1
            else:
                groupLabels = dict(zip(df[id],df[group]))
                if len(set(df[id]).difference(set(groupLabels.keys())))>0:
                    raise ValueError(f'Some values of {id} are not assigned to a group in the groupLabels lookup')
                else:
                    df_est['group'] = df_est[id].apply(lambda x: groupLabels[x])
        ...
        return RegressionResultsWrapper(self.modelstring,a_x,m_y,m_ys,W,y,id,exog_labels,v_theta_ini,bnds,optim_output,variance,variance_sand,df_est,df_mg_est,N,T,K,robust)
