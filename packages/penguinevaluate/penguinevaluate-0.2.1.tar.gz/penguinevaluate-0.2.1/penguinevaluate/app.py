...
def run_inference(input_isbn: ISBN13, start_date: Query(dt.date), end_date: Query(dt.date)) -> Tuple[List, pd.DataFrame, Bool]:
    ...
    """
    comp_titles, df_actual_predicted, causation_check = get_actual_predicted_special(
        input_isbn,
        start_date,
        end_date)    
    ...
    return {
        ...
        "actual_predicted": df_actual_predicted.to_dict(orient='records', lines=False),
        "causation_check": causation_check
        }