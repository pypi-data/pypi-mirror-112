...
def get_chords_by_beat(midi_obj, split_char='_'):
    markers = midi_obj.markers
    n_beats = int(np.around(max([marker.time for marker in markers]) / 480) + 1)
    chord_by_beat = [Chord() for _ in range(n_beats)]
    for marker in markers:
        if marker.text[:6] != 'rhythm':
            try:
                beat = np.around(marker.time / 480).astype(int)
                chord_by_beat[beat] = Chord(marker.text.replace(split_char, '')).simplify()
            except KeyError:
                pass
