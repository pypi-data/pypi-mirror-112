...
class Evaluate():
    ...
    def feature_importance(self, df: pd.DataFrame, 
        ...
        with tqdm(desc='Calculating Feature Importance', total=8, unit='step', file=sys.stdout, leave=False) as progress:
            ...
            if catboost_parameters:
                # check to see if catboost parameter arguments all match the approved keys
                unsupported_params = [p for p in list(catboost_parameters.keys()) if p not in approved_catboost_params]
            ...
            if not catboost_parameters.get("iterations"):
                # This defaults iterations to 300. If a user passes in iterations, we'll use that
                # if not, 300 is our baseline
                catboost_parameters["iterations"] = 300
            catboost_parameters["random_seed"] = 123

            # Build model and measure performance
            if target_distinct_vals > 2:
                catboost_parameters["custom_metric"] = ['R2','RMSE','MAPE']
                model = catboost.CatBoostRegressor(**catboost_parameters)
                try:
                ...
                catboost_parameters["custom_metric"] = ['AUC','Logloss','Precision','Recall']
                model = catboost.CatBoostClassifier(**catboost_parameters)
