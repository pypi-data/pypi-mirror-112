...
def tsbasismom(start_date, **kwargs):
    ...
    for i in range(start_index, len(calendar) - 1):
        t_date = calendar[i]
        lookback_date = calendar[i - window_days]
        lookback_date2 = calendar[i - 2*window_days]

        data_t = data[
            np.array(data['TDATE'] == t_date)
            & np.array(data['VOL'] > kwargs['min_volume'])
            ][['TDATE', 'UCODE', 'CCODE', 'CCODE2', ]]
        ...
        data_t = pd.merge(
            data_t,
            kwargs['data_raw'][['TDATE', 'CCODE', 'CLOSE', 'DDATE']].rename(columns={'CLOSE': 'CLOSE1t0'}),
            on=['TDATE', 'CCODE'],
            how='left'
        )
        
        data_t = pd.merge(
            data_t[['EXCHANGE', 'PCODE', 'UCODE', 'CLOSE', 'RY']],
            data_lookback[['UCODE', 'CLOSE', 'RY']].rename(columns={'CLOSE': 'CLOSE0', 'RY': 'RY0'}),
            on='UCODE', how='left'
        )
        ...
        data_t['POS'] = (data_t['RY'] - data_t['RY0']) / abs(data_t['RY'] - data_t['RY0']) #* -1
        data_t = pd.merge(
            data_t, data_tomorrow[['UCODE', 'CLOSE']].rename(columns={'CLOSE': 'CLOSE1'}), on='UCODE', how='left'
        )
        data_t['RETURN'] = (data_t['CLOSE1'] / data_t['CLOSE'] - 1)
