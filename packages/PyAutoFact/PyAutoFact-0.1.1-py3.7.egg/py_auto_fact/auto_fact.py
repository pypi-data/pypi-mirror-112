...
"""
def conv_sgd(weight, rank, num_iter=10):
...
def factorize_module(module, rank, ignore_lower_equal_dim, fact_led_unit, solver, num_iter):
    if type(module) == nn.Linear:
        ...
        # Initialize matrix
        if solver == 'svd':
            U, V = linear_svd(module.weight, rank)
            ...
            U, V = linear_nmf(module.weight, rank)
            led_module.led_unit[0].weight.data = U # Initialize U
            led_module.led_unit[1].weight.data = V # Initialize V
            led_module.led_unit[1].bias = child.bias
        elif solver == 'snmf':
            U, V = linear_snmf(module.weight, rank)
            led_module.led_unit[0].weight.data = U # Initialize U
            led_module.led_unit[1].weight.data = V # Initialize V
            led_module.led_unit[1].bias = child.bias
...
def auto_fact(module, rank, deepcopy=False, ignore_lower_equal_dim=True, fact_led_unit=False, solver='random', num_iter=10):
    ...
    if type(module) in [nn.Linear, nn.Conv1d, nn.Conv2d, nn.Conv3d]:
        return factorize_module(child, rank, ignore_lower_equal_dim, fact_led_unit, solver, num_iter)
