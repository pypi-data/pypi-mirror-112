from src.map_analyser import maplib as ml
import numpy as np
...
#     prace = area.coords_zipped
#     return orientation(prace[0], prace[1], prace[2])
#
#
# def coords_orientation(prace):
#     return orientation(prace[0], prace[1], prace[2])
#
#
# def debug_coords_orientation(prace):
#     a,b,c = prace[0], prace[1], prace[2]
...
#
# def TODOlists_polygon_to_areas(polygon, area):
#     ret = []
#     for i in range(len(polygon)):
#         p = polygon[i]
#         if area.HOLES_ORIENTATION == \
#                 orientation(ml.Coord(p[0][0], p[0][1]),
#                     ml.Coord(p[1][0], p[1][1]),
#                     ml.Coord(p[2 % len(p)][0], p[2 % len(p)][1])):
#             ret[-1].add_hole(p, area.HOLES_ORIENTATION)
#         else:
#             tmp = area.copy()
#             tmp.drop_holes()
#             tmp.set_coords([(point[0], point[1]) for point in p], area.ORIENTATION)
#             ret.append(tmp)
#
#     return ret
