...
from .util import ParseError, Coord, CoordsLists
        ...
        for s_id, s in symbols.items():
            if s.symbol_appearance:
                if s.symbol_appearance['color'] == 'Calculate':
                    sub_symbols = [symbols[key] if isinstance(key, int) else key for key in s.symbol_appearance['symbols']]
                    ...
                    elif s.code in ['508.1', '508.2', '508.3', '508.4']: # '502.2', '508.1', '508.2', '508.3', '508.4', '509', '511.2'
