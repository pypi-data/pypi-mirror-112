...
class HcsCom:
    ...
    def __init__(self, port: Union[str, serial.Serial]):
        ...
        m = sha1()
        m.update(self.port_name.encode())
        lockfile_path = "/var/lock/hcscom_port_{0}.lock".format(m.hexdigest())
        self.lockfile_path = None
        try:
            self.acquire_lockfile(lockfile_path)
    ...
    def acquire_lockfile(self, lockfile_path):
