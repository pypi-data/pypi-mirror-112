...
class DomainNet(ImageList):
    ...
    Parameters:
        ...
        - **domain** (str): The domain to create dataset. Choices include ``'c'``:clipart, \
    ...
    def __init__(self, root: str, domain: str, evaluate: Optional[bool] = False, download: Optional[float] = False, **kwargs):
        assert domain in self.image_list
        phase = "test" if evaluate else "train"
        data_list_file = os.path.join(root, "image_list", "{}_{}.txt".format(self.image_list[domain], phase))
