...
class UserCredentialsCache(object):
    ...
    _XDG_BASE = Path.expanduser(Path("~/.config"))

    @property
    def cache_file_dir(self) -> Path:
        """Directory where we'll save the cached creds"""
        if os.name == "nt":  # needed for Windows.
            config_path = Path(os.environ["APPDATA"])
        else:
            config_path = self._XDG_BASE
    ...
    def cache_file_path(self) -> Path:
        ...
        user_credentials_cache_path = self.cache_file_dir / self._CACHE_DIRNAME / self._USER_CREDENTIALS_FILENAME
    ...
    def load(self) -> Optional[GoogleCredentials]:
        ...
        Returns:
            user_credentials or None if we could not not load them for some reason (eg. they didn't exist, they were malformed, etc.)
        """
