...
def export_mlp_policy_to_json(train_agent: Trainer) -> None:
    """Export the model policy associated with a given agent.

    :param test_agent: Agent to evaluate on a single simulation.

    :returns: Path of json file of policy model.
    """
    # Get trained policy
    policy = train_agent.get_policy()

    # Make sure the machine learning framework is torch
    assert policy.framework == 'torch', (
        "This model is only supported for 'torch' framework.")
    # Get observation normalization
    observation_space = policy.observation_space
    obs_filter = train_agent.workers.local_worker().filters["default_policy"]
    if isinstance(obs_filter, NoFilter):
    ...
    # Get action normalization
    env = train_agent.env_creator({**train_agent.config["env_config"]})
    action_space = env.unwrapped.action_space
    action_mean = (action_space.high + action_space.low) / 2.0
    action_std = (action_space.high - action_space.low) / 2.0

    # Extract model info
    n_layers = len(policy.model._hidden_layers) + 2
    activation = policy.model._hidden_layers[0]._model[1]
    model_sizes, model_weights, model_biases = [], [], []
    for layer in policy.model._hidden_layers:
        layer_weights = layer._model[0].weight.detach().cpu().numpy()
        layer_biases = layer._model[0].bias.detach().cpu().numpy()
        model_sizes.append(layer_weights.shape[1])
        model_weights.append(layer_weights.tolist())
        model_biases.append(layer_biases.tolist())
    final_layer = policy.model._logits
    layer_weights = final_layer._model[0].weight.detach().cpu().numpy()
    layer_biases = final_layer._model[0].bias.detach().cpu().numpy()
    model_sizes.append(layer_weights.shape[1])
    model_sizes.append(layer_weights.shape[0])
    model_weights.append(layer_weights.tolist())
    model_biases.append(layer_biases.tolist())
    ...
    model_data["output_parameters"] = {
        ...
        "size": len(action_mean)
    }
    model_data["conv_parameters"] = {}
    model_data["dense_parameters"] = {
        "n_layers": n_layers,
        "layer_sizes": model_sizes,
        "weights": model_weights,
        "biases": model_biases,
        "activation": activation.__class__.__name__.upper()
    }

    # Export model info to json
    json_path = f"{train_agent.logdir}/policy_{train_agent.iteration}.json"
