...
def get_parameter(key, key_option, stage, revision, history, query_all, query, format, verbosity, global_scope, project_scope, config_override, working_dir):
    ...
    def validate_command_usage():
        ...
        if history:
            defaultQuery = '{Revisions: Revisions[*].{RevisionId: RevisionId, Date: Date, Value: Value, Scope: Scope, Origin: Origin}}'
        else:
...
@click.option('--config-override', 'config_override', metavar='<key>=<value>,...', required=False, help=f"{CLI_PARAMETERS_HELP['common']['config']}")
@click.option('-w','--working-dir', metavar='<path>', type=click.Path(exists=True, file_okay=False), required=False, help=f"{CLI_PARAMETERS_HELP['common']['working_dir']}")
def get_template(key, key_option, stage, revision, history, include_contents, query_all, query, format, verbosity, global_scope, project_scope, config_override, working_dir):
    ...
    def validate_command_usage():
        ...
        key = validate_at_least_one_provided([key, key_option], ["KEY", "'-k' / '--key'"])
        validate_not_all_provided([history, revision], ["'--history'", "'--revision'"])
        # validate_not_all_provided([history, revision], ["'--history'", "'--include-contents'"])
        if history:
            if include_contents:
                defaultQuery = '{Revisions: Revisions[*].{RevisionId: RevisionId, Date: Date, Scope: Scope, Origin: Origin, Contents: Contents}}'
            else:
                defaultQuery = '{Revisions: Revisions[*].{RevisionId: RevisionId, Date: Date, Scope: Scope, Origin: Origin}}'
        else:
