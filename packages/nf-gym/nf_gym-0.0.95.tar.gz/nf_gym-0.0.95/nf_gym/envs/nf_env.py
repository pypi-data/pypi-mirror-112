...
class NFEnv(gym.Env):
    ...
    def step(self, action):
        ...
        rews[1]=0.6*(math.exp(-height_diff*5.0)-1)
            
        speed=obs[5:7]
        speed_diff=np.linalg.norm(speed)
        if self.debug:
            print("speed",speed,"speed_diff",speed_diff)
        rews[2]=0.2*(math.exp(-speed_diff*2.0)-1)
        
        power=sum([abs(t) for t in taus])
        if self.debug:
            print("taus",taus)
            print("power",power)
        rews[3]=0.2*(math.exp(-power*0.03)-1)
        ...
        if abs(pos[0])>0.9 or abs(pos[1])>0.9:
            done=True
        pitch=obs[0]
        if abs(pitch)>0.9:
            done=True
