...
class NFEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if prev_action is not None:
            action_diff=np.linalg.norm(np.array(action[:4])-np.array(prev_action[:4]))
            if self.debug:
                print("action_diff=%s prev_action=%s action=%s"%(action_diff,prev_action[:4],action[:4]))
        else:
            action_diff=0
        rews[2]=0.4*(math.exp(-action_diff*5)-1)
        ...
        if done and not self.done:
            ...
            if self.debug:
                ...
                act_diff=[r[2] for r in self.all_rews]
                """
