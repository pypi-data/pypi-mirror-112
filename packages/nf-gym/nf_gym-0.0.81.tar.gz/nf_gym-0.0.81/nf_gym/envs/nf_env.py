...
class NFEnv(gym.Env):
    ...
    def __init__(self,show_gui=False,fall_rew=-10,motion_rew=0,const_rew=2,height_rew=0,rand_force=None,mp4_path=None,foot_friction_lat=1,foot_friction_spin=1,pitch_rew=-1,roll_rew=-1,distance_rew=0,max_distance=0,power_rew=-0.1,min_height=0.5,max_pitch=0,max_roll=0,target_height=1.7,rand_force_freq=1000,speed_x_rew=-0.1,speed_y_rew=-0.1,speed_z_rew=0,speed_pitch_rew=0,speed_roll_rew=0,speed_yaw_rew=0,max_steps=5000,debug=False,base_mass=4,min_actions=[-1]*12,max_actions=[1]*12,target_speed_x=0,target_speed_y=0,action_sim_steps=int((1.0/TIME_STEP)/40),rand_action_delay=0,max_torque=100,kp_max=200,kd_max=3,max_episode_len=400):
        self.step_counter = 0
        if show_gui:
            mode=p.GUI
        else:
    ...
    def get_obs(self):
        ...
        obs.append(self.last_torques[0]/self.max_torque) # torque
        s=p.getJointState(self.bot_id, self.joints["umot_to_mbar_r"])
        obs.append(s[0]/math.pi) # pos
        obs.append(s[1]/math.pi) # vel
        obs.append(self.last_torques[1]/self.max_torque) # torque
        s=p.getJointState(self.bot_id, self.joints["lmot_to_uleg_l"])
        obs.append(s[0]/math.pi) # pos
        obs.append(s[1]/math.pi) # vel
        obs.append(self.last_torques[2]/self.max_torque) # torque
        s=p.getJointState(self.bot_id, self.joints["lmot_to_uleg_r"])
        obs.append(s[0]/math.pi) # pos
        obs.append(s[1]/math.pi) # vel
        obs.append(self.last_torques[3]/self.max_torque) # torque
    ...
    def step(self, action):
        ...
        rews[1]=0.8*(math.exp(-height_diff*5.0)-1) # XXX
        """
        
        """
        lforce_diff=abs(100-l_force)
        rews[1]=0.45*(math.exp(-lforce_diff*0.03)-1)

        rforce_diff=abs(100-r_force)
        rews[2]=0.45*(math.exp(-rforce_diff*0.03)-1)
        """

        lfoot_diff=abs(0.015-lfoot_z)
        rews[1]=0.25*(math.exp(-lfoot_diff*20)-1) # XXX

        rfoot_diff=abs(0.015-rfoot_z)
        rews[2]=0.25*(math.exp(-rfoot_diff*20)-1) # XXX

        lknee_diff=abs(0.15-lknee_z)
        rews[3]=0.25*(math.exp(-lknee_diff*20)-1) # XXX

        rknee_diff=abs(0.15-rknee_z)
        rews[4]=0.25*(math.exp(-rknee_diff*20)-1) # XXX

        power=sum([abs(t) for t in taus])
        if self.debug:
            print("taus",taus)
            print("power",power)
        #rews[5]=0.1*(math.exp(-power*0.01)-1) # XXX
        rews[5]=0
        ...
        if self.step_counter==self.max_episode_len:
            ...
            if self.debug:
                ...
                lfoot=[r[1] for r in self.all_rews]
                rfoot=[r[2] for r in self.all_rews]
                lknee=[r[3] for r in self.all_rews]
                rknee=[r[4] for r in self.all_rews]
                ...
                axs[3][0].plot(lfoot,label="lfoot")
                axs[3][0].plot(rfoot,label="rfoot")
                axs[3][0].plot(lknee,label="lknee")
                axs[3][0].plot(rknee,label="rknee")
                axs[3][0].plot(rknee,label="power")
