...
def ultroid_cmd(allow_sudo=should_allow_sudo(), **args):
    ...
    def decorator(func):
        ...
        async def wrapper(ult):
            ...
            if current in ["official", "dualmode"]:
                to_check = sudoers()
                if current = "dualmode":
                    to_check = [ultroid_bot.uid, *sudoers())
                if not ult.out:
                    if not allow_sudo or (str(ult.sender_id) not in to_check):
                        return
        ...
        if manager and "manager" in type:
            current = "manager"
            ultroid_bot.asst.add_event_handler(wrapper, events.NewMessage(**args))
        if current != "manager" and "dualmode" in type:
            current = "dualmode"
            ultroid_bot.asst.add_event_handler(wrapper, events.NewMessage(**args))
        try:
