...
class PlusDataFrame():
    ...
    def set_datatypes(self):
        ...
        try:
            ...
            for key, val in self.datatypes_dict.items():   
                if val in ('float','int','int32','int64'):
                    logging.info(f'Replacing commas with full stops in numbers for attribute: {key}')
                    self.df[f'{key}_new'] = str(self.df[key]).replace(',','.')
                    self.df[key] = self.df[f'{key}_new'].fillna(self.df[key])
                    del self.df[f'{key}_new']
                if val in ('bool'):
                    self.df[f'{key}_new'] = self.df[key].str.replace({'J':True, 'Y':True, 'Yes':True, 'Ja':True, 'N':False, 'No':False, 'Nee':False})
                    self.df[key] = self.df[f'{key}_new'].fillna(self.df[key])
                    del self.df[f'{key}_new']
