        ...
        try:
            ...
            for key, val in self.datatypes_dict.items():   
                if val == 'float':
                    ...
                    self.df[key] = self.df[f'{key}_new'].fillna(self.df[key]).as_type('float')
                    del self.df[f'{key}_new']
                elif val == 'bool':
                    numpy_array = self.df.loc[:, key]
                    numpy_array.replace({'J':True, 'Y':True, 'Yes':True, 'Ja':True, 'N':False, 'No':False, 'Nee':False})
                    self.df[key] = numpy_array
                    numpy_array = None
                elif val in ('int','int16','int32','int64') and not isinstance(self.df[key], ('int','int16','int32','int64')):
                        self.df[key] = self.df[key].as_type('float')
                else:
                    pass
