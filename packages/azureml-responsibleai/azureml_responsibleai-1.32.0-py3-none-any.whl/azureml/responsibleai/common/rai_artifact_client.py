...
class RAIArtifactClient(ChainedIdentity):
    ...
    def __init__(self, run: Run, datastore_name: Optional[str] = None, **kwargs):
    ...
    def generate_storage_path(self, artifact_area_path: str,
                              upload_id: str, artifact_type: str) -> str:
        """Create a path in storage for a particular artifact.

        The result will be something like:
        {artifact_area_path}/{upload_id}/{artifact_type}.json

        :param artifact_area_path: The top level name for rai
                                   artifact for example 'causal',
        ...
        """
        fmt = "{0}/{1}/{2}.json"
        result = fmt.format(artifact_area_path,
                            upload_id,
                            artifact_type)
        return result

    def upload_single_object(self, target: Any, artifact_area_path: str,
                             upload_id: str, artifact_type: str) -> Dict[str, str]:
        ...
        :param artifact_type: The type of the artifact being created
        :type artifact_type: str
        :returns: Single dictionary with a single entry of
                 'prefix' and storage path.
        :rtype: Dict[str,str]
        """
        storage_path = self.generate_storage_path(artifact_area_path,
                                                  upload_id, artifact_type)
    ...
    def download_single_object(self, artifact_area_path: str,
                               upload_id: str, artifact_type: str) -> Any:
