...
def standalone() -> None:
    ...
    init_params: dict = {}
    if args.topology:
        init_params["topology_path"] = ValidatedFilePath(args.topology)
    if args.sequence:
        init_params["sequence_path"] = Path(args.sequence)
    if args.kubeconfig:
        init_params["kubeconfig_path"] = Path(args.kubeconfig)
    if args.components:
        init_params["components_dir_path"] = ValidatedDirPath(args.components)
    if args.mermaid_export_path:
        init_params["components_dir_path"] = Path(args.mermaid_export_path)
