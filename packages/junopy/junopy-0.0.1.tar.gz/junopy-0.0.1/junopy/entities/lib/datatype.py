...
class ValidateValue:
    ...
    def __init__(self, default=None, context=None, required=False, max=None, name=None, type=None, format=None, key=None):
        ...
        self.name = name
        if name:
            self.type = getattr(__import__(f'junopy.entities.{name.lower()}', fromlist=[name]), name)
        else:
            self.type = None
        self.format = format
        self.key = key
        self._value = None if not self.__class__ or not 'List' in self.__class__.__name__ else ListType(self.type, context, key)
    ...
    def __reset__(self):
        self._value = None if not self.__class__ or not 'List' in self.__class__.__name__ else ListType(self.type, self.context, self.key)
            ...
            if not isinstance(data, datetime.datetime):
                try:
                    ...
                    if "unconverted data remain" not in str(e):
                        raise Exception(f"is not datetime or string is not correct format, expected [{self.format}]")
                    pass
        super().__setattr__(attr, data)
