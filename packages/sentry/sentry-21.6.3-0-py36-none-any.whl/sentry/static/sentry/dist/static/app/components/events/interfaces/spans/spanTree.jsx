...
var SpanTree = /** @class */ (function (_super) {
    ...
    SpanTree.prototype.render = function () {
        ...
        var _b = spans.reduce(function (acc, payload, index) {
            ...
            var key = getSpanID(span, "span-" + index);
            ...
            acc.spanTree.push(<SpanBar key={key} organization={organization} event={waterfallModel.event} spanBarColour={spanBarColour} spanBarHatch={type === 'gap'} span={span} showSpanTree={!waterfallModel.hiddenSpanGroups.has(getSpanID(span))} numOfSpanChildren={numOfSpanChildren} trace={waterfallModel.parsedTrace} generateBounds={generateBounds} toggleSpanTree={_this.toggleSpanTree(getSpanID(span))} treeDepth={treeDepth} continuingTreeDepths={continuingTreeDepths} spanNumber={spanNumber} isLast={isLast} isRoot={isRoot} isCurrentSpanFilteredOut={false} showEmbeddedChildren={payload.showEmbeddedChildren} toggleEmbeddedChildren={payload.toggleEmbeddedChildren} fetchEmbeddedChildrenState={payload.fetchEmbeddedChildrenState}/>);
            return acc;
        }, {
