...
def get_backtest_params_iter_from_stats_list(
    ...
    for stats in stats_list:
        ...
        if engine_kwargs[vt_symbol_key] != stats.symbols_info.symbols:
            logger.warning(
                f"{stats} engine_kwargs['{vt_symbol_key}']={engine_kwargs[vt_symbol_key]} "
                f"与 stats.symbols_info.symbols={stats.symbols_info.symbols} 不一致，以后者为准")
            engine_kwargs[vt_symbol_key] = stats.symbols_info.symbols
