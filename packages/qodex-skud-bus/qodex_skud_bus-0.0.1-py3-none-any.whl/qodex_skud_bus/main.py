...
from qodex_skud_bus.exceptions import exceptions


class QodexSkudBus:
    def __init__(self, skud_ip, skud_port, points_list=None, skud_test_mode=False, *args, **kwargs):
        ...
        self.skud_sdk = SigurSDK(skud_ip, skud_port, 1, 2, 3, 4, test_mode=skud_test_mode)
    ...
    def get_api_support_methods(self):
        methods = {
            ...
            'get_point_all_states_dict': {'method': self.get_point_all_states_dict},
    ...
    def init_points(self, pointlist):
        """ Инициировать экземпляры классов для всех точек доступов, заданных суперклассу атрибутом points_list"""
        for point_dict in pointlist:
            point_type = point_dict['point_type']
            point_superclass = functions.get_point_value(point_type, 'superclass')
            point_object = point_superclass(point_dict['point_num'], self.skud_sdk)
    ...
    def get_point_state(self, point_name_str: str, *args, **kwargs):
        ...
        point_state = point_dict['point_object'].get_current_state()
        return point_state

    def get_point_all_states_dict(self, point_name_str: str, *args, **kwargs):
        point_dict = functions.get_point_dict(self.points_list, 'point_name', point_name_str)
        point_state = point_dict['point_object'].get_states_dict()
        return point_state

    def lock_point(self, point_name_str: str, *args, **kwargs):
        point_dict = functions.get_point_dict(self.points_list, 'point_name', point_name_str)
        point_dict['point_object'].set_point_locked()
        return point_dict['point_name']

    def unlock_point(self, point_name_str: str, *args, **kwargs):
        point_dict = functions.get_point_dict(self.points_list, 'point_name', point_name_str)
        point_dict['point_object'].set_point_unlocked()
        return point_dict['point_name']

    def normal_point(self, point_name_str: str, *args, **kwargs):
        point_dict = functions.get_point_dict(self.points_list, 'point_name', point_name_str)
        point_dict['point_object'].set_point_normal()
        return point_dict['point_name']
...
if __name__ == '__main__':
    points_list = [
        {'point_name': 'EXTERNAL_GATE', 'point_num': 2, 'point_type': 'EXTERNAL_GATE', 'position': 'external'},
        {'point_name': 'INTERNAL_GATE', 'point_num': 1, 'point_type': 'INTERNAL_GATE', 'position': 'internal'},
        {'point_name': 'INTERNAL_PHOTOCELL', 'point_num': 3, 'point_type': 'INTERNAL_PHOTOCELL', 'position': 'internal'},
        {'point_name': 'EXTERNAL_PHOTOCELL', 'point_num': 4, 'point_type': 'EXTERNAL_PHOTOCELL', 'position': 'external'},
    ]
    qsb = QodexSkudBus('192.168.100.109', 3312, points_list=points_list, skud_test_mode=False)
