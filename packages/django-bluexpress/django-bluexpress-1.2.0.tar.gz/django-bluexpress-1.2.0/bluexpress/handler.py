...
class BluexpressHandler:
    ...
    def get_events(self, raw_data):
        """
            ...
            return [{}, {}, ...]
        """
        document = self._extract_document(raw_data.get('tracking_data'))
        ...
        events = [
            ...
            } for prick in document['listaPinchazosNacionales']['pinchazo']
        ]
    ...
    def get_status(self, raw_data):
        ...
        """
        document = self._extract_document(raw_data.get('tracking_data'))
        macroestates = document['listaMacroestados']['macroestado']
        status = macroestates[-1].get('nombre')
        is_delivered = False

        if not status:
            status = macroestates[-2].get('nombre')
    ...
    def _extract_document(self, tracking_data):
        """
            Extract document from xml string.
        """

        parse = xmltodict.parse(tracking_data)
        json_data = json.loads(json.dumps(parse))
        envelope = self._extract_full_key(json_data, 'Envelope')
        envelope_data = json_data[envelope]
        body = self._extract_full_key(envelope_data, 'Body')
        body_data = envelope_data[body]
        response = self._extract_full_key(body_data, 'responseObtenerDocumento')
        response_data = body_data[response]
        return response_data['listaDocumento']['documento']

    def _extract_full_key(self, json_data, search_key):
        """
            Extract full key from tag.
        """
        for data in json_data:
            if search_key in data.split(':'):
                return data
