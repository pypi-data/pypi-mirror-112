...
class ReservationSync:
    ...
    def _create_guest_profile(self, hotelId, guest):
        try:
            guestId = 0
            ...
            _profile_data = self.retrieve_data(hotelId, self.token, _url)
            if _profile_data:
                idObj = dict()
    ...
    def _get_guest_list(self, hotelId, record):
        ...
        for item in record.get('reservationGuests'):
            _guest_profile = self._create_guest_profile(hotelId, item)
            self._update_master_profile(_guest_profile)
    ...
    def _update_master_profile(self, record):
        ...
        if details.get('profileType') == 'Guest':
            ...
            guest.nationalityCode = details.get('customer').get('citizenCountry').get('code')
            guest.nationalityCountry = details.get('customer').get('citizenCountry').get('value')
