...
class ManagerWorkersLoading(ManagerWorkersBase):
    ...
    def manage_nodeset_loading(self, assigned_no_of_cores):
        ...
        if no_of_free_cores > 0:
            ...
            log.info(
                f"INIT WORKERS WAITING: {self._get_workers('initial', tag=self.worker_tag_nodesets)}"
            )
            for next_worker in self._get_workers(
                ...
                new_worker_started = True
                next_worker.timer.__enter__()
                log.debug(f"CALL START ON NS LOADING {next_worker}")
                next_worker.start()
    ...
    def _order_drains(self, assigned_no_of_cores):
        ...
        next_workers = [
            w
            for w in self._get_workers("initial", tag=self.worker_tag_relsets)
        ...
        log.info(
            f"CHECK DRAINING WAITING WORKERS. count={len(self.drain_waiting_workers)}"
        )
        for drain_wait_worker in self.drain_waiting_workers:
            ...
            log.info(f"drains_done? before blockcheck: {drains_done}")
            ...
            for nodeset in relset_meta_details.target_nodeSets:
                if nodeset in nodesets_currently_worked_on:
                    ...
                    tmp_drain_waiting_workers.append(drain_wait_worker)
            # END FIX EDGECASE
            log.info(f"drains_done? after blockcheck: {drains_done}")
            if drains_done:
                log.info(
