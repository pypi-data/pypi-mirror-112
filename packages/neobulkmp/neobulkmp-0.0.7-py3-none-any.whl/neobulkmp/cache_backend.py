...
class CacheInterface():
    ...
    * Mongo DB - https://www.mongodb.com/de
    ...
    def fetch_RelSets(self, relset_meta: SetsMetaBase, drain_order_id: str) -> graphio.RelationshipSet:
    ...
    def list_SetsMeta(self, set_type: Type[Union[graphio.NodeSet, graphio.RelationshipSet]] = None) -> List[SetsMetaBase]:
    ...
    def report_SetLoaded(self, nodeset: Union[graphio.NodeSet, graphio.RelationshipSet]):
        raise NotImplementedError
...
class RedisCache(CacheInterface):
    ...
    class DrainOrder:
        ...
        @classmethod
        def create_order(cls, relset_meta: RelationSetMeta, cache: "RedisCache") -> "RedisCache.DrainOrder":
            ...
            drain_order = cls(id=drain_order_id, relset_meta=relset_meta, nodesets_meta=relset_meta.target_nodeSets,
                              buckets_key_base=buckets_key_base, relset_bucket_key=[], nodesets_buckets_keys=[], time_of_drain_order=drain_timestamp, preceding_overlapping_drain_orders=[])

            return drain_order._materialize_drain_order(cache)

        @ classmethod
        def get_by_order_id(cls, drain_order_id: uuid.UUID, cache: "RedisCache") -> "RedisCache.DrainOrder":
        ...
        @classmethod
        def list_orders(cls, cache, status=None, nodeset_meta: SetsMetaBase = None) -> List["RedisCache.DrainOrder"]:
            p = cache.redis.pipeline()
            ...
            for res in results:
                for drain_id, raw_drain_order in res.items():
                    order: RedisCache.DrainOrder = jsonpickle.loads(
                        raw_drain_order)
                    if nodeset_meta is None or (nodeset_meta is not None and nodeset_meta.key in [ns.key for ns in order.nodesets_meta]):
                        orders.append(order)
        ...
        def list_open_nodesets_meta(cls, cache: "RedisCache"):
            open_nodesets_meta = []
            all_drained_buckets_keys = [bk.decode("utf-8") for bk in cache.redis.lrange(
            ...
            for drain_order in all_open_drain_orders:
                for drain_nodeset_meta, bucket_key in zip(drain_order.nodesets_meta, drain_order.nodesets_buckets_keys):
                    if bucket_key not in all_drained_buckets_keys + ["DRAIN_EMPTY_PLACEHOLDER_KEY"] and drain_nodeset_meta not in open_nodesets_meta:
                        open_nodesets_meta.append(drain_nodeset_meta)
            return list(reversed(open_nodesets_meta))

        @classmethod
        def get_drain_bucket_keys(cls, cache: "RedisCache", graphset_meta: SetsMetaBase = None, set_type: Type[Union[graphio.NodeSet, graphio.RelationshipSet]] = None):
            bucket_keys = []
            all_open_drain_orders = cls.list_orders(cache, "OPEN")
            for drain_order in all_open_drain_orders:
                ...
                if set_type in [None, graphio.NodeSet]:
                    for bucket_key in drain_order.nodesets_buckets_keys:
                        if bucket_key != "DRAIN_EMPTY_PLACEHOLDER_KEY":
                            if graphset_meta is None or (graphset_meta is not None and graphset_meta in drain_order.nodesets_meta):
                                bucket_keys.append(bucket_key)
                if set_type in [None, graphio.RelationshipSet]:
                    if graphset_meta is None or (graphset_meta is not None and graphset_meta == drain_order.relset_meta):
        ...
        def _set_status(self, cache: "RedisCache", status: str):
            if status == "READY":
                ...
                p = cache.redis.pipeline()
                p.multi()
                p.hdel(cache.DRAIN_ORDERS_KEY,
                       self.id)
                p.hset(cache.DRAIN_ORDERS_READY_KEY, self.id,
                       jsonpickle.dumps(self))
                p.execute()
            elif status == "FINISHED":
                # Delete drain order
                p = cache.redis.pipeline()
                p.multi()
                p.hdel(cache.DRAIN_ORDERS_READY_KEY,
                       self.id)
                p.execute()
                # Delete related nodeset done entries
                to_be_deleted_entries = []
                for key in cache.redis.lrange(cache.DRAIN_ORDERS_NODESETS_DONE_KEY, 0, -1):
                    key = key.decode()
                    if self.id in key:
                        to_be_deleted_entries.append(key)
                for key in to_be_deleted_entries:
                    cache.redis.lrem(
                        cache.DRAIN_ORDERS_NODESETS_DONE_KEY, 0, key)

            else:
                raise ValueError(
                    f"Expected 'READY' or 'FINISHED' got {status}")
        ...
        def get_nodeset(self, cache: "RedisCache", nodeset_meta) -> graphio.NodeSet:
            ...
            for bucket_key, drain_nodeset_meta in zip(self.nodesets_buckets_keys, self.nodesets_meta):
                if nodeset_meta.key == drain_nodeset_meta.key:
                    if bucket_key != "DRAIN_EMPTY_PLACEHOLDER_KEY":
                        nodeSet_parameter_dict = cache._parse_key(
                            nodeset_meta.key, cache.NODESET_PREFIX)
                        ns = graphio.NodeSet.from_dict({**nodeSet_parameter_dict, **{
                            "nodes": cache._fetch_list_range(bucket_key, cache.COMBINE_SET_COUNT, True)}})
                        ns.drain_order = self
        ...
        def get_relset(self, cache: "RedisCache") -> graphio.NodeSet:
            ...
            rs = graphio.RelationshipSet.from_dict(
                {**relSet_parameter_dict, **{"relationships": cache._fetch_list_range(self.relset_bucket_key, None, True)}})
            rs.drain_order = self
            rs.drain_bucket_key = self.relset_bucket_key
            rs.nodeset_meta = self.relset_meta
            return rs

        def _set_nodeset_status_drained(self, cache: "RedisCache", nodeset: graphio.NodeSet):
            if nodeset.drain_order.id == self.id:
                # is bucket not a placeholder and is bucket not empty:
                if nodeset.drain_bucket_key != "DRAIN_EMPTY_PLACEHOLDER_KEY" and cache.redis.llen(nodeset.drain_bucket_key) == 0:
                    log.debug(
                        f"SET nodeset {nodeset.labels} for {self.id} as drained")
                    cache.redis.rpush(
        ...
        def are_nodesets_drained(self, cache: "RedisCache"):
            ...
            for other_drain_orders in self.preceding_overlapping_drain_orders:
                if not other_drain_orders.are_nodesets_drained(cache):
                    return False
            # Check self
            all_drained_buckets_keys = [bucketkey.decode("utf-8") for bucketkey in cache.redis.lrange(
                cache.DRAIN_ORDERS_NODESETS_DONE_KEY, 0, -1)]

            self_bucket_keys = [
                bucketkey for bucketkey in self.nodesets_buckets_keys if bucketkey != "DRAIN_EMPTY_PLACEHOLDER_KEY"]
        ...
        @classmethod
        def set_graphset_drained(cls, cache: "RedisCache", graphset: Union[graphio.NodeSet, graphio.RelationshipSet]):
            if isinstance(graphset, graphio.NodeSet):
                ...
                # /Sanity check
                graphset.drain_order._set_nodeset_status_drained(
                    cache, graphset)
        ...
        def _materialize_drain_order(self, cache):
            ...
            for drain_id, pickled_drain_order in other_drain_orders.items():
                ...
                if [other_ns for other_ns in other_drain_order.nodesets_meta if other_ns in self.nodesets_meta]:
            ...
            cache.redis.hmset(
                cache.DRAIN_ORDERS_KEY, {self.id: jsonpickle.dumps(self)})
    ...
    def _fetch_list_range(self, key: str, range_size: int = None, parse_and_combine=True) -> List:
    ...
    def fetch_NodeSets(self, nodeset_meta: NodeSetMeta) -> graphio.NodeSet:
        ...
        # Check if there are any open drainOrders on this type if NodeSet
        ...
        nodeSet_parameter_dict = self._parse_key(
            nodeset_meta.key, self.NODESET_PREFIX)
        ...
        return graphio.NodeSet.from_dict({**nodeSet_parameter_dict, **{"nodes": self._fetch_list_range(nodeset_meta.key, self.COMBINE_SET_COUNT, True)}})
    ...
    def fetch_RelSets(self, relset_meta: SetsMetaBase, drain_order_id: str) -> graphio.RelationshipSet:
        ...
        drain_order = RedisCache.DrainOrder.get_by_order_id(
            drain_order_id, self)
        ...
        return graphio.RelationshipSet.from_dict(
            {**relset_meta_dict, **{"relationships": self._fetch_list_range(drain_order.relset_bucket_key)}})

    def list_SetsMeta(self, set_type: Type[Union[graphio.NodeSet, graphio.RelationshipSet]] = None) -> List[SetsMetaBase]:
        meta_data = []
        if set_type in [None, graphio.NodeSet]:
            # fetch all keys begining with NODESET_PREFIX and pack each into an SetsMetaBase instance
            nodeset_keys = [key.decode(
                "utf-8") for key in self.redis.keys(f"{self.NODESET_PREFIX}*")]
            for nkey in nodeset_keys:
                meta_entry = SetsMetaBase(
                    key=nkey, total_size_bytes=0, type=graphio.NodeSet)
                meta_entry.total_size_bytes = self._get_NodeSet_real_size(
                    meta_entry)
        ...
        if set_type in [None, graphio.RelationshipSet]:
            ...
            nodeset_keys = [key.decode(
                "utf-8") for key in self.redis.keys(f"{self.RELSET_PREFIX}*")]
            meta_data.extend([SetsMetaBase(key=nkey, total_size_bytes=self.redis.memory_usage(
                nkey), type=graphio.RelationshipSet) for nkey in nodeset_keys])
        # Sort by priority (Drainorders first, then by size)
        meta_data.sort(key=operator.attrgetter(
            "total_size_bytes"), reverse=True)

        if set_type in [None, graphio.NodeSet]:
            # Move nodesets with drain order to the front
            for open_drain_nodeset_meta in list(reversed(RedisCache.DrainOrder.list_open_nodesets_meta(cache=self))):
                # check if nodeset appears in list of nodeset with a drainOrder, if yes get index this nodeset in meta_data list
                index = next((i for i, md in enumerate(meta_data)
                              if md.key in open_drain_nodeset_meta.key), None)
    ...
    def get_NodeSetMeta(self, nodeset_meta: SetsMetaBase) -> NodeSetMeta:
        node_meta = NodeSetMeta(key=nodeset_meta.key,
                                total_size_bytes=self._get_NodeSet_real_size(nodeset_meta))
        node_meta.labels = self._parse_key(
            nodeset_meta.key, self.NODESET_PREFIX)["labels"]
        node_meta.relations = []
        ...
        for rel in all_rel_sets:
            ...
            if set(node_meta.labels) in [set(rel_data["start_node_labels"]), set(rel_data["end_node_labels"])]:
    ...
    def get_RelSetMeta(self, relset_meta: SetsMetaBase) -> RelationSetMeta:
        rel_meta = RelationSetMeta(
            key=relset_meta.key, total_size_bytes=self.redis.memory_usage(relset_meta.key))
        rel_key_data = self._parse_key(
            rel_meta.key, self.RELSET_PREFIX)
        ...
        for nodeset in all_node_sets:
            ...
            if set(nodeset_data["labels"]) in [set(rel_key_data["start_node_labels"]), set(rel_key_data["end_node_labels"])]:
                rel_meta.target_nodeSets.append(nodeset)
        return rel_meta

    def order_RelSetDrain(self, relset_meta: SetsMetaBase) -> str:

        drain_order: self.DrainOrder = self.DrainOrder.create_order(
            relset_meta, self)
        log.info(
            f"Ordered Drain id:'{drain_order.id}' on {drain_order.relset_meta.key}")
    ...
    def is_drain_done(self, drain_order: Union["RedisCache.DrainOrder", str]) -> bool:
        ...
        if isinstance(drain_order, str):
            d_order = RedisCache.DrainOrder.get_by_order_id(
                drain_order, self)
        ...
        if d_order.get_status(self) == "READY":
            log.info(
                f"Drain done: id:'{d_order.id}' on {d_order.relset_meta.key}")
            return True
        else:
            return False

    def report_SetLoaded(self, graphset: Union[graphio.NodeSet, graphio.RelationshipSet]):
    ...
    def get_logs(self) -> List[logging.LogRecord]:
        return [jsonpickle.loads(record)
                for record in self.redis.lrange("LOG_LIST", 0, -1)]
