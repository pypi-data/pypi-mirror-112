...
class RedisCache(CacheInterface):
    ...
    class DrainOrder:
        ...
        def is_empty(self, cache):
            ...
            for bucket in self.nodesets_buckets_keys:
                if (
                    ...
                    log.info(f"NOT FINISHED BECAUSE BUCKJETS EXISTS: {bucket}")
                    return False
            for preceding_order in self.preceding_overlapping_drain_orders:
                if preceding_order.get_status(cache) == "OPEN":
                    log.info(
                        f"NOT FINISHED BECAUSE PRECEDING EXISTS: {preceding_order.id}"
                    )
                    return False
            return True
        ...
        def get_status(self, cache):
            if self.is_empty(cache):
                log.info(f"Fix empty drain order {self.id}")
            ...
            results = p.execute()
            if results[0]:
                log.info(f"I AM {self.id} OPEN")
                return "OPEN"
            if results[1]:
                log.info(f"I AM {self.id} READY")
                return "READY"
            log.info(f"I AM {self.id} FINISHED because {results}")
            return "FINISHED"
        ...
        def are_nodesets_drained(self, cache: "RedisCache"):
            ...
            for other_drain_orders in self.preceding_overlapping_drain_orders:
                if not other_drain_orders.are_nodesets_drained(cache):
                    log.info(f"PRECEDING not finsihed {other_drain_orders.id}")
                    return False
            # Check self
            ...
            if set(self_bucket_keys).issubset(set(all_drained_buckets_keys)):
                ...
                log.info(
                    f"\nset(self_bucket_keys):\n{set(self_bucket_keys)} \n set(all_drained_buckets_keys): \n {set(all_drained_buckets_keys)}"
                )
                return False
    ...
    def order_RelSetDrain(self, relset_meta: SetsMetaBase) -> str:
        ...
        log.info(
            f"Ordered Drain id by proc:{os.getpid()}:'{drain_order.id}' on {drain_order.relset_meta.key}"
        )
    ...
    def is_drain_done(self, drain_order: Union["RedisCache.DrainOrder", str]) -> bool:
        ...
        if d_order.get_status(self) in ["READY"]:
            log.info(f"Drain done: id:'{d_order.id}' on {d_order.relset_meta.key}")
            return True
        else:
            log.info(f"DRAIN NOT DONE with status {d_order.get_status(self) }")
            return False
