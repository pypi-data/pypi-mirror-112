...
class MessageBus(AbstractMessageHandler):
    ...
    @property
    def msa(self) -> Optional[AbstractJ3]:
        return self._msa

    @msa.setter
    def msa(self, new_msa: Optional[AbstractJ3]):
...
def on_command(etype: Type[C]) -> Callable[[F], F]:
    ...
    def _wrapper(func: F) -> F:
        ...
        if handler:
            raise J3Error(f"Handler already exists for {etype}: {handler}")
        messagebus.register(etype, func)
        return func
...
class MessageBroker(AbstractMessageHandler):
    ...
    async def main(self):
        if not self.msa.allow_external_event:
            raise J3Error("External events are not allowed!")
