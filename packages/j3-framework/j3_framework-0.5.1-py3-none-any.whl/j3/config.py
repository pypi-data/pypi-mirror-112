...
from j3.core import AbstractJ3, AbstractMessageBroker
...
def load_setupcfg(path: Path) -> Optional[J3SetupConfig]:
    if (path / "setup.cfg").exists():
        ...
        if "j3" in config:
            return J3SetupConfig(**config["j3"])
    return None


@dataclass
class J3Config(AbstractJ3):
    """J3 App 설정."""
    ...
    @staticmethod
    def load_from_config(path=Path(".")) -> J3Config:
        ...
        if (module_path / "config" / "__init__.py").exists() or (
            ...
            UserConfig = cast(Type[J3Config], getattr(conf_module, "Config"))
