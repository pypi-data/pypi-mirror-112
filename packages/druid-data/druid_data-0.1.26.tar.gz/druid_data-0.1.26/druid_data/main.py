...
class DynamoDBGlobalConfiguration:
    ...
    def get_connection(self):
        if self.ddb is None: # Connect to the database if ddb is None
            try:
...
class DynamoCrudRepository(CrudRepository):
    ...
    def __init__(
        ...
        self.ddb = global_values.get_connection()
        if dynamo_table_name is not None:
            ...
            if global_values.single_table_name_env_var in os.environ:
                self.table_name = os.environ[global_values.single_table_name_env_var]
            else:
