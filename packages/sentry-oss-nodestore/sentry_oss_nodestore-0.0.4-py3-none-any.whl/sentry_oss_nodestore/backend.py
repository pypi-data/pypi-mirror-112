...
class OSSNodeStorage(NodeStorage):
    ...
    def __init__(self, bucket_name=None, prefix=None, endpoint=None, oss_access_key_id=None, oss_secret_access_key=None, max_retries=3):
        # self.max_retries = max_retries
        # self.bucket_name = bucket_name
        # self.client = boto3.client(
        #     's3', endpoint_url=endpoint, aws_access_key_id=aws_access_key_id, aws_secret_access_key=aws_secret_access_key)
        self.prefix = '' if not prefix else prefix     # 不要prefix的时候oss的读取性能是最好的
        self.max_retries = max_retries
        self.bucket_name = bucket_name
