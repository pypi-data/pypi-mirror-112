...
class GlobalPointerLabelProcessor(ABCLabelProcessor):
    ...
    def inverse_transform(self,labels,lengths,mapping_list = None,threshold=0):
        result = []
        ...
        for index,(scores ,sequece_length )in enumerate(zip(labels,lengths)):
            ...
            for l, start, end in zip(*np.where(scores > threshold)):
                if mapping_list:
                    key = (mapping_list[index][start][0], mapping_list[index][end][-1])
                    value = {}
                    value['label'] = self.idx2vocab[l]
                    value['score'] = scores[l][start][end]
                    if key not in score_dict:
                        score_dict[key] = value
                    else:
                        if value['score'] >= score_dict[key]['score']:
                            score_dict[key] = value
            ...
            for key,value in score_dict.items():
                entities.append((key[0], key[1], value['label']))
