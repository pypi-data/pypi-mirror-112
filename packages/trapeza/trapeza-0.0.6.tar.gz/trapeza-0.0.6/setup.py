...
ext_dtoa = Extension("trapeza.arithmetics.dtoa",
                     sources=["trapeza/arithmetics/dtoa.pyx", "src/ryu/d2s.c"],
                     include_dirs=["src/emyg/"] + ["src/ryu/"],
                     language='c')
...
if os.name == 'nt':
    ...
    ext_libmpdec = Extension("trapeza.arithmetics.libmpdec",
                             ...
                             library_dirs=["src/win/mpdecimal-2.5.1/libmpdec/"],
                             include_dirs=["src/"] + ["src/win/mpdecimal-2.5.1/"] +
                                          ["src/win/mpdecimal-2.5.1/libmpdec/"],
                             extra_objects=["src/win/mpdecimal-2.5.1/libmpdec/libmpdec-2.5.1.dll.lib"],
                             language='c')
else:
    # linux or macOS
    ext_libmpdec = Extension("trapeza.arithmetics.libmpdec",
                             sources=["trapeza/arithmetics/libmpdec.pyx"],
                             library_dirs=["src/linux/mpdecimal-2.5.1/libmpdec/"],
                             include_dirs=["src/"] + ["src/linux/mpdecimal-2.5.1/"] +
                                          ["src/linux/mpdecimal-2.5.1/libmpdec/"],
                             language='c')
...
def setup_call(_extensions, _install_requires, _cmdclass, _ver_str):
    setup(ext_modules=_extensions,
          ...
          data_files=[('trapeza/arithmetics', ['src/win/mpdecimal-2.5.1/libmpdec/libmpdec-2.5.1.dll'])],
