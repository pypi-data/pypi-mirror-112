...
class SotoonParams(click.Parameter, StatefulParam):
    def __init__(self, name, placeholder, description, is_required, default_value):
        ...
        # TODO: envvar is left, should I set it here?

        # Added:
        self.is_required = is_required
        self.description = f"{description}{'' if default_value is None else f' [default:  {default_value}]'}" \
...
class SotoonArgument(click.Argument, SotoonParams):
    def __init__(self, name, placeholder, description, is_required, default_value, param_type):
        ...
        SotoonParams.__init__(self, name, placeholder, description, is_required, default_value)


class SotoonOption(click.Option, SotoonParams):
    def __init__(self, name, placeholder, description, is_required, default_value, param_type, short_name=None):
        param_dcl = [name, f"--{name}"]
        param_dcl.append(f"-{short_name}") if short_name else None
        click.Option.__init__(self, param_dcl, type=param_type)
        SotoonParams.__init__(self, name, placeholder, description, is_required, default_value)


class SotoonBooleanFlag(click.Option, SotoonParams):
    def __init__(self, name, placeholder, description, is_required, default_value, short_name=None):
        param_dcl = [name, f"--{name}/--no-{name}"]
        param_dcl.append(f'-{short_name}') if short_name else None
        # No type needed for this kind of bool option, this is fixed in click 8.0 (issue no.: #1287)
        click.Option.__init__(self, param_dcl, help=description)
        SotoonParams.__init__(self, name, placeholder, description, is_required, default_value)
