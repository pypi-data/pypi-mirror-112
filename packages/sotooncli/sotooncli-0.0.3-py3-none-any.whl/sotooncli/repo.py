...
from sotooncli.command import GroupCommand, ExecutableCommand
from sotooncli.param import SotoonOption, SotoonArgument, SotoonBooleanFlag
...
class CliRepo:
    ...
    def _get_group(data, parent=None):
        ...
        group = GroupCommand(data['name'], data['description'], data['capsule'], args, opts, parent)
    ...
    def _get_command(data, parent):
        args, opts = CliRepo._get_param(data)
        cmd = ExecutableCommand(data['name'], data['description'], data['capsule'], args, opts, parent)
        return cmd
    ...
    def _get_param(data):
        ...
        for param_data in data["args"]:
            param_type = CliRepo._get_param_type(param_data["type"], param_data["choices"])
            if param_data["is_positional"]:
                if param_type == LIST:
                    arg = SotoonArgument(name=param_data['name'], placeholder=param_data['placeholder'],
                                         description=param_data['description'], is_required=param_data['is_required'],
                                         default_value=param_data['default'], param_type=param_type)
                else:
                    arg = SotoonArgument(name=param_data['name'], placeholder=param_data['placeholder'],
                                         description=param_data['description'], is_required=param_data['is_required'],
                                         default_value=param_data['default'], param_type=param_type)
                ...
                if param_type == click.BOOL:
                    opt = SotoonBooleanFlag(name=param_data['name'], placeholder=param_data['placeholder'],
                                            description=param_data['description'],
                                            is_required=param_data['is_required'],
                                            default_value=param_data['default'], short_name=param_data['short_name'])
                else:
                    opt = SotoonOption(name=param_data['name'], placeholder=param_data['placeholder'],
                                       description=param_data['description'], is_required=param_data['is_required'],
                                       default_value=param_data['default'], param_type=param_type,
    ...
    def _get_param_type(type_str, choices):
        if choices:
            return ChoiceParamType(CliRepo._get_param_type(type_str, None), choices=choices)
        elif type_str == "int":
            return click.INT
            ...
            click.echo(f"CLI doesn't support type \"{type_str}\", you need to upgrade your CLI.")
            sys.exit(1)
