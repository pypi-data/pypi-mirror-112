...
def get_params_from_file_option():
    def call_back(ctx, param, value):
        ...
        try:
            ...
            raise click.BadParameter(f"Could not open file: {value}: {get_strerror(e)}", ctx=ctx)

    option = click.Option(["--params-file", "-f"], help=f"Path to YAML file for reading params.",
                          callback=call_back, metavar="YAML_FILE")
    return option
