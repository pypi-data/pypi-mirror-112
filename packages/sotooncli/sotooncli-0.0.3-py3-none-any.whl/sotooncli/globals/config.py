...
from sotooncli.sotoon.config import get_active_config_path
...
class ConfigOption(click.Option, StatefulParam):
    def __init__(self):
        click.Option.__init__(self, ["config", "--config"],
                              help=f"Path to config file. [default: currently active config path]",
                              callback=self.get_config, metavar="CONFIG_PATH")
        StatefulParam.__init__(self, "config", self.type)
        self.active_config_path = None
        try:
            active_config = get_active_config_path()
            self.active_config_path = active_config
        except Exception:
            return
    ...
    def get_config(self, ctx, param, config_path):
        ...
        try:
            ...
            raise click.BadParameter(f"Could not open file: {config_path}: {get_strerror(e)}", ctx=ctx)
        return config_path
