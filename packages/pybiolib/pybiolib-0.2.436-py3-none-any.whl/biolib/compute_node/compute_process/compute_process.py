...
class ComputeProcess:
        ...
        while True:
            try:
                ...
                if package == b'STOP':
                    ...
                    for instance in self._lfs_instances:
                        instance.unmount()
                    break
    ...
    def _compute(self, module_input):
        try:
            ...
            for lfs in self.module.get('large_file_systems', []):
                lfs_instance = LargeFileSystem(job_id=self.job_id, public_id=lfs['public_id'], to_path=lfs['to_path'])
                lfs_instance.mount()
