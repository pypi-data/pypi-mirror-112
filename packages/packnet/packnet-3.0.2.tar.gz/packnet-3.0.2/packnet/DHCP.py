...
class Header(Frame):
    def __init__(self, *args, **kwargs):
        ...
        self.op = INT( 1, size=1 )
        self.ht = INT( 1, size=1 )
        self.hl = INT( 6, size=1 )
        self.hops = INT( 0, size=1 )
        self.id = INT( randint(0, 0xffffffff), size=4 )
        self.elapsed = INT( 0, size=2 )
        self.flags = INT( 0x0, size=2 )
        self.clientip = IP()
        self.yourip = IP()
        self.serverip = IP()
        self.gatewayip = IP()
        self.clientmac = MAC()
        self.padding = b"\00" * 202
        self.magiccookie = INT( 0x63825363, size=4 )
        self.protocol = None
...
cookieserver    8           n           ip
lprserver       9           n           ip
impressserver   10          n           ip
rsrclocserver   11          n           ip
hostname        12          n           name
bootfilesize    13          2           int
meritdumpfile   14          n           str
domainname      15          n           name
swapserver      16          4           ip
rootpath        17          n           str
extensionpath   18          n           str
ipforwarding    19          1           int
nonlclsourcert  20          1           int
