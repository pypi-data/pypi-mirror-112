...
from azureml.automl.core.shared._diagnostics.automl_events import RunFailed
...
def fail_run(
    ...
    _log_error_event(run, interpreted_exception)
...
def _log_error_event(run: RunType, exception: BaseException) -> None:
    ...
    run_failed_event = RunFailed(run_id=run.id, error_code=error_code, error=error_str)
    event_logger.log_event(run_failed_event)
