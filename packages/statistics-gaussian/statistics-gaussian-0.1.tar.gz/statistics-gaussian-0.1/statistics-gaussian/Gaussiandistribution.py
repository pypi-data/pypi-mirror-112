...
class Gaussian():
    ...
    def calculate_stdev(self, sample=True):
        ...
        num   = [xi -  self.mean for xi in self.data]
        num_squared = [x**2 for x in num]
        ...
        std = math.sqrt(sum(num_squared) / n)
        self.stdev = std
    ...
    def pdf(self, x):
        ...
        pdf = (1/math.sqrt(self.stdev**2 * 2 * math.pi)) * math.exp((-1 * (x-self.mean) ** 2) / self.stdev**2 * 2 )
        return pdf
