...
from parallelformers.transformers.modeling_blenderbot_small import BlenderbotSmallAttention
...
class BlenderbotSmallEncoderPolicy(Policy):
    ...
    def replace_modules():
        return {
            "BlenderbotSmallAttention": BlenderbotSmallAttention,
        }
...
class BlenderbotSmallDecoderPolicy(Policy):
    ...
    def replace_arguments(config, world_size):
        return {
            # 1. reduce hidden size
            "self_attn.embed_dim":
                config.d_model // world_size,
            "encoder_attn.embed_dim":
                config.d_model // world_size,

            # 2. reduce number of heads
            "self_attn.num_heads":
                config.decoder_attention_heads // world_size,
