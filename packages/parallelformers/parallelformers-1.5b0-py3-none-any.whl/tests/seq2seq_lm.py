...
class TestForCausalLM(unittest.TestCase):
    ...
    def test_forward(self, model, tokens, use_pf, fp16):
        ...
        torch.save(output.encoder_hidden_states, f"output-encoder-{'pf' if use_pf else 'non-pf'}-{'fp16' if fp16 else 'fp32'}.pt")
        torch.save(output.decoder_hidden_states, f"output-decoder-{'pf' if use_pf else 'non-pf'}-{'fp16' if fp16 else 'fp32'}.pt")
        print()
...
if __name__ == '__main__':
    ...
    gpus = [_ for _ in range(
        args.gpu_from,
        args.gpu_to + 1,
    )]

    tokens = tokenizer(
        "Kevin is <mask> man in the <mask>",
        return_tensors="pt",
    )
