...
def to_str(var: any,
    ...
    if is_float(var):
        ...
        if chars != 0:  # -1 for all, or x>0 for x chars
            if fp >= 0:
                new_v = round_list(var.tolist(), fp=fp)
            else:
                new_v = var.tolist()
...
def round_list(li: (list, tuple), fp: int = 3, warn: bool = False):
    ...
    if fp >= 0 and len(li) > 0:
        if all((is_int(item) or is_float(item)) for item in li):  # 1d list of int and floats
            if all(is_int(item) for item in li):  # if all ints - no rounding
                f_format = '{:,}'
            else:
                f_format = '{:,.%xf}' % fp
            new_li = [f_format.format(li_item) for li_item in li]
