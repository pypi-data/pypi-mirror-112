...
class PdBaseModel(BaseModel):
    ...
    def __init__(
        ...
        self.pairs_indices_colors = self.model_cfg['pairs_indices_colors']
    ...
    def draw_detections(
        ...
        for detection in detections:  # each detection is a pose - currently supports just 1
            ...
            for i, pair in enumerate(self.pairs_indices):
                partA, partB = pair
                if xys[partA] != self.NOT_FOUND_PAIR and xys[partB] != self.NOT_FOUND_PAIR:
                    edge_c = pyplt.get_BGR_color(self.pairs_indices_colors[i])
...
class TflPdModelPoseNet(TflPdModel):
    def __init__(
        ...
        super().__init__(save_load_dir=save_load_dir, model_name=model_name, threshold=threshold,
                         allowed_joint_names=allowed_joint_names, check_type=False)
...
class Cv2PdModel(PdBaseModel):
    ...
    def __init__(self,
        ...
        if device == "cpu" or True:
            self.pose_net.setPreferableBackend(cv2.dnn.DNN_TARGET_CPU)
        elif device == "gpu":
...
class Cv2PdModelCocoMultiPoses(Cv2PdModel):
    def __init__(self,
        ...
        super().__init__(
            ...
            threshold=threshold, in_dims=in_dims, scalefactor=scalefactor, mean=mean, swapRB=swapRB, crop=crop,
            check_type=False, device=device
        )
    ...
    def extract_results(
        ...
        for i, pose in enumerate(poses):
            if ack:
                print('{}\t{}'.format(tabs * '\t', mt.to_str(pose, 'pose {}'.format(i))))
            detection_d = {
                'joint_names_list': [],
                'joint_ids_list': [],
                'joint_x_y_list': [],
            ...
            for joint_id, joint_name in self.joint_names.items():
                ...
                if ack:
                    d_msg = '{}\t\tpose {}: {}({}): xy={}, score=({}%)'
                    print(d_msg.format(tabs * '\t', i, joint_name, joint_id, detection_d['joint_x_y_list'][-1],
                                       detection_d['score_percentages_list'][-1]))
            detections.append(detection_d)
        return detections
