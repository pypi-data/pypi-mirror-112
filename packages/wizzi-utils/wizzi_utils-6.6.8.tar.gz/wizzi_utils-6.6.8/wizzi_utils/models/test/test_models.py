...
def od_or_pd_Model_image_test(
    ...
    for model in models:
        ...
        cvt.add_text(cv_img_clone, header='{:.2f} FPS'.format(fps.get_fps()), pos=(cv_img_clone.shape[1] - 100, 20),
                     text_color='r', with_rect=False, bg_font_scale=2)
        ...
        if success:
            for model, fps in zip(models, fps_list):
                ...
                cvt.add_header(cv_img_clone, header=model.model_name, loc=pyplt.Location.BOTTOM_LEFT.value,
                               bg_font_scale=1)
                cv_img_clone = cv2.resize(cv_img_clone, dis_size, interpolation=cv2.INTER_AREA)
                cvt.add_text(cv_img_clone, header='{:.2f} FPS'.format(fps.get_fps()),
                             pos=(cv_img_clone.shape[1] - 100, 20), text_color='r', with_rect=False, bg_font_scale=2)
                cv_imgs_post.append(cv_img_clone)
            if mp4 is not None:
