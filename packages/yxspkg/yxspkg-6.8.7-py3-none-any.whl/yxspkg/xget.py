...
def get_html_url_re_bytes(html):
    ...
    return s
def get_html_url(html,filetype='text',ftype='bs4',title_dict = {'h':'href','s':'src'}):
    if filetype == 'path':
        try:
            html = open(html).read()
        except:
...
#     if p.is_file():
#         i = p
#         for h in href.finditer(open(i,'rb').read()):
#             yield i.parent,h.group()
#         yield False,False
#     else:
#         htmls = 0
...
def main(args,parallel,url,ss,ftypes,decode_name,convert_links):
    if not url:
        url = args[0]
