...
class SFTPManager:
  ...
  def create_connection(self):
    try:
      if self.privateKey is not None:
        self.sftp = pysftp.Connection(host=self.host, username=self.username, privateKey=self.privateKey, cnopts=self.cnopts)
  ...
  def downloadDir(self, remotePath:str, localPath:str, filterOptions: dict = dict()) -> List[str]:
    ...
    """    
    try:
      self.__downloadDir__(self.sftp, self.sftp.listdir(), localPath, filterOptions)
    except Exception as e:
      logger.error(e)
      raise e
  
  def uploadFile(self, localFilename:str, remoteFilename:str):
