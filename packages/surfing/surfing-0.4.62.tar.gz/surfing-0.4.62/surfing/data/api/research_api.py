...
from surfing.data.wrapper.mysql import TLDatabaseConnector
...
class ResearchApi(metaclass=Singleton):
    ...
    def get_index_quote(self,start_date,end_date, index_ids):
        ...
        with BasicDatabaseConnector().managed_session() as db_session:
            try:
                ...
                df = pd.read_sql(query.statement, query.session.bind).rename(columns=rename_dic)
                return df
        ...
        try:
            ...
            with RawDatabaseConnector().managed_session() as db_session:
                latest = db_session.query(
                        func.max(EmIndexComponent.datetime)
                    ).filter(
                        EmIndexComponent.em_id.in_(df.em_id),
                ).one_or_none()[0]
                query = db_session.query(
                            EmIndexComponent.index_id,
                            EmIndexComponent.stock_list,
                            EmIndexComponent.datetime,
                        ).filter(
                            EmIndexComponent.datetime == latest,
                            EmIndexComponent.em_id.in_(df.em_id),
                        )
                df2 = pd.read_sql(query.statement, query.session.bind)
                df = df.drop(labels=['em_id'], axis=1)
                df = df.set_index('index_id').join(df2.set_index('index_id'))
                df = df.reset_index().rename(columns=rename_dic)
                return df
    ...
    def get_fund_info(self, fund_id = None ,desc_name = None ,wind_class_1 = None ,wind_class_2 = None ,
        ...
        with BasicDatabaseConnector().managed_session() as db_session:
            try:
                ...
                if (size != None):
                        query_size = query_size.filter(
                                Fund_size_and_hold_rate.size >= size[0],
                                Fund_size_and_hold_rate.size <= size[1],
                        )
                df_size = pd.read_sql(query_size.statement, query_size.session.bind)
                if(fund_id != None):
                        fund_id = fund_id + list(set(df_size['fund_id'].values.tolist()))                       
                else:
                        fund_id = list(set(df_size['fund_id'].values.tolist()))
                query = db_session.query(
                            FundInfo.fund_id,
                            FundInfo.desc_name,
                ...
                df = pd.merge(df,df_size,how = 'left',on = 'fund_id')
                return df
            except Exception as e:
    ...
    def get_future_quote(self, future_ids, start_date, end_date):
        '''期货行情'''
        with RawDatabaseConnector().managed_session() as db_session:
            try:
                query = db_session.query(
    ...
    def get_pf_nav(self, start_date, end_date, record_cds:list=[]):
        '''私募净值查询接口'''
        with TLDatabaseConnector().managed_session() as db_session:
            try:              
                ...
                df = pd.read_sql(query.statement, query.session.bind)
                dff = pd.merge(df1, df, left_on='SECURITY_ID', right_on='SECURITY_ID')
                return dff
