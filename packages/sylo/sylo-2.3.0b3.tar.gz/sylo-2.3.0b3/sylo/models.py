...
class Database:
    ...
    def sql_to_df(self, sql: List, cols: List, index_col: str = None):
    ...
    def initial_data_load(self, durations: Durations):
        session_data, = self.select(f"SELECT * FROM sessions where session_date = '{TODAY_STR}';"),
        poms_data, = self.select(f"SELECT * FROM poms where poms_date = '{TODAY_STR}';"),
        try:
