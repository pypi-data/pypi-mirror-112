...
class Task:
    ...
    def write_task_to_file(self):
        ...
        fields = [int(max_id) + 1, self.date, self.description, self.effort, self.complete]
...
class Sylo:
    ...
    def edit_task_menu(self, dates: List):
        ...
        while usr_input.lower() == "n":
            ...
            task.effort = input(
                display.col(f'Effort (in'
                            f' {display.add_plurality(display.pom_name)}) >> ',
                            'yellow')
            )
    ...
    def input_loop(self):
        ...
        while response.lower() == "t":
            self.edit_task_menu([YESTERDAY_STR, TODAY_STR, ])
        while response == "":
