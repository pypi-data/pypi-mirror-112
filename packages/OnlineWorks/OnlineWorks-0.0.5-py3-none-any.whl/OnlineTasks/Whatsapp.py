...
def send_whatsapp_message_to_group(group_id, message, time_hour, time_min, wait_time=20, print_waitTime=True, browser_name='deafult', browser_path='deafult'):
    """
    It will send a whatsapp message to group
    """
    if time_hour not in range(0, 25) or time_min not in range(0, 60):
    ...
    time.sleep(sleeptm)
    if browser_name == 'deafult' and browser_path == 'deafult':
        web.open_new_tab('https://web.whatsapp.com/accept?code='+group_id)
    else:
        p = r"{}".format(browser_path)
        web.register(browser_name, None, web.BackgroundBrowser(p))
        web.get(browser_name).open_new_tab('https://web.whatsapp.com/accept?code='+group_id)
    time.sleep(2)
    width, height = pg.size()
